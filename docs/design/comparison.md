...
Fibers gives `guard` an activation-scoped interpretation. Each structural use is prepared independently, so `together({ g, g })` evaluates a reused guard twice, while one outer guard may deliberately construct a shared option. A guard returned by `and_then` is evaluated when that particular provisional progression activates. Its builder receives the prefix values directly as varargs and returns an explicit residual operation. The residual is then retained while the same progression is searched, suspended or reconstructed. This resembles CML pre-synchronisation preparation while accounting for Fibers' multi-step speculative worlds.

The main difference is CML's single selected synchronisation point. Work may be arranged before or after that point, but a compound protocol must still decide which communication constitutes commitment. `fibers.and_then` keeps earlier state changes and exchanges provisional until the complete right-hand operation and all recruited participants close.
