...
local function fill_from_combo(plan, combo)
  ...
  for i = 1, n do
    local c = combo[i]
    plan.transaction = append_all(plan.transaction, c.trans)
...
function Plan.try_from_world(rt, world, cursor)
  ...
  local prepared, reason = Resource.prepare_combo(combo, Candidate.raw_resolved, Candidate.resolve)
  if reason then return nil, reason or 'resource-not-fresh' end

  return fill_from_combo({ _token = PLAN_TOKEN, tag = 'commit-plan', prepared_resources = prepared }, combo)
end
