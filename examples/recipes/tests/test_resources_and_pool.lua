...
local function test_keyed_together_put_supplies_get()
  local rt = new_runtime()
  local m = Keyed.new():label('keyed-together')
  local rows
  rt:spawn_raw(function()
...
local function test_keyed_each_put_does_not_supply_get()
  local rt = new_runtime()
  local m = Keyed.new():label('keyed-each')
  local rows
  rt:spawn_raw(function()
...
local function test_keyed_remove_and_get_share_parent_value()
  local rt = new_runtime()
  local m = Keyed.from({ a = 'A', b = 'B' }):label('keyed-remove-each')
  local rows
  rt:spawn_raw(function()
...
local function test_keyed_take_returns_value()
  local rt = new_runtime()
  local m = Keyed.from({ a = 'A' }):label('keyed-take')
  local v
  rt:spawn_raw(function()
...
local function test_priority_queue_order_and_handoff_laws()
  local rt = new_runtime()
  local pq = PriorityQueue.new(math.huge):label('pq-order')
  rt:spawn_raw(function()
  ...
  local pq2 = PriorityQueue.new(math.huge):label('pq-law')
  ...
  local pq3 = PriorityQueue.new(math.huge):label('pq-each')
...
local function test_pool_acquire_release_and_retirement()
  ...
  assert_eq((holders(pool.leases, 'a') or {}).u1, 'lease')
  ...
  assert_status(rt2:run(), 'found')
  assert_nil((holders(pool.leases, 'a') or {}).u1)
  assert_eq(perform_op(pool.items:get_op('a')).item, 'A')
  assert_eq(entries(pool.idle).a.value, 'a')
...
local function test_pool_each_add_does_not_supply_acquire_but_together_does()
  local pool = Pool.new():label('pool-law')
  local rt = new_runtime()
  local rows
  rt:spawn_raw(function()
  ...
  assert_eq(entries(pool.idle).x.value, 'x')

  local pool2 = Pool.new():label('pool-together')
  local rt2 = new_runtime()
  local rows2
  rt2:spawn_raw(function()
  ...
  assert_eq(entries(pool2.idle).x, nil)
  assert_eq(holders(pool2.leases, 'x').u, 'lease')
end
...
local function test_keyed_take_then_put_replaces()
  local rt = new_runtime()
  local m = Keyed.from({ a = 'A' }):label('keyed-replace-after-remove-present')
  local old
  rt:spawn_raw(function()
...
local function test_pool_close_constrains_acquire_under_together_and_each()
  local pool = Pool.new():label('pool-close-law')
  local rt = new_runtime()
  local rows
  rt:spawn_raw(function()
  ...
  assert_eq(State.value(pool.open), false)
  assert_eq(perform_op(pool.items:get_op('a')).item, 'A')
  assert_eq(entries(pool.idle).a.value, 'a')

  local pool2 = Pool.new():label('pool-close-law-each')
  local rt2 = new_runtime()
  local rows2
  rt2:spawn_raw(function()
  ...
  assert_eq(State.value(pool2.open), false)
  assert_eq(perform_op(pool2.items:get_op('a')).item, 'A')
  assert_eq(entries(pool2.idle).a.value, 'a')
end
