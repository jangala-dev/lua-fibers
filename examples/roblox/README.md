...
| `01_button_choice.client.luau` | LocalScript | attach an event-driven application to one Lifetime and close it through Closure |
...
| `04_server_shutdown.server.luau` | Script | cancel and close the root through `BindToClose` |
...
explicit application Closure; example 06 adds RunService-phase policy above
the same boundary.
