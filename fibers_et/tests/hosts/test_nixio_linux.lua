...
local ok_mod, NixioHost = pcall(require, 'fibers.host.nixio_linux')
...
if not NixioHost.is_supported() then
  return Common.skip('tests/hosts/test_nixio_linux.lua', 'nixio backend not available')
end

local ok_nixio, nixio = pcall(require, 'nixio')
if not ok_nixio or type(nixio) ~= 'table' then
  return Common.skip('tests/hosts/test_nixio_linux.lua', 'nixio module not available')
end
if type(nixio.pipe) ~= 'function' then
  return Common.skip('tests/hosts/test_nixio_linux.lua', 'nixio.pipe unavailable')
end
...
with_host_pipe('nixio_linux:readiness', Common.readiness_smoke)
with_host_pipe('nixio_linux:write-readiness', Common.write_readiness_smoke)
with_host_pipe('nixio_linux:readiness-beats-timeout', Common.readiness_beats_timeout_smoke)
with_host_pipe('nixio_linux:timeout-beats-unready', Common.timeout_beats_unready_smoke)

print('tests/hosts/test_nixio_linux.lua: ok')
