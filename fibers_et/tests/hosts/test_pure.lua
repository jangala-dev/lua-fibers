...
do
  ...
  Common.assert_truthy(st.waits and st.waits[1] and st.waits[1].kind == 'source', 'pending status should report source wait')
end
...
do
  ...
  Common.assert_truthy(Host.has_non_time_waits({ { kind = 'time', deadline = 1 }, { kind = 'source' } }))
end
