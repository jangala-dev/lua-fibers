...
do
  ...
  feed:readable()
  drive_until(rt, function() return ok == true end, 'bounded readiness should resume', true)
...
do
  ...
  rt:spawn_raw(function() read_seen = rt:perform(src:readable_op()) end, 'read-waiter')
  rt:spawn_raw(function() write_seen = rt:perform(src:writable_op()) end, 'write-waiter')
  assert_status(rt:run(), 'pending')
  feed:writable()
  drive_until(rt, function() return write_seen == true end, 'write readiness should resume')
  assert_eq(write_seen, true)
  assert_nil(read_seen, 'read waiter should remain pending after write readiness')
  feed:readable()
  drive_until(rt, function() return read_seen == true end, 'read readiness should resume')
end
...
do
  ...
  feed:clear('read')
  local seen
  rt:spawn_raw(function() seen = rt:perform(src:readable_op()) end, 'clear-waiter')
  local st = rt:run()
...
do
  local rt = Runtime.new()
  local region = fibers.Region.new('readiness-authority-region')
  ...
  drive_until(rt, function() return read_err == 'read_reset' end, 'read error should come from backend read')
  backend:fail_writes('write_reset')
  backend:mark_writable()
  drive_until(rt, function() return write_err == 'write_reset' end, 'write error should come from backend write')
  assert_nil(read_val)
  assert_nil(n)
end
...
do
  ...
  local a, b
  feed:readable()
  rt:spawn_raw(function() a = rt:perform(src:readable_op()) end, 'level-a')
  rt:spawn_raw(function() b = rt:perform(src:readable_op()) end, 'level-b')
  drive_until(rt, function() return a == true and b == true end, 'level readiness should be reusable while set')
end

-- Stale read readiness is safe: backend read may still return would_block and no stream bytes appear.
do
  local rt = Runtime.new()
  local region = fibers.Region.new('stale-readiness-region')
...
do
  local rt = Runtime.new()
  local region = fibers.Region.new('bounded-ready-pump-region')
