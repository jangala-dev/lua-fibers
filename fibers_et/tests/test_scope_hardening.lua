...
do
  ...
  rt:spawn_raw(function()
    scope:run(function(s)
      ...
      fibers.perform(s:raw_region():admit_op(fibers.Region.Owned.item(h, function()
...
do
  ...
  fibers.run(function(root)
    ...
    local stream = fibers.stream(backend, { name = 'friendly-stream' })
    owner_was_root = stream.owner == root:raw_region()
  end)
  assert_eq(owner_was_root, true, 'fibers.stream should bind to the current scope')
end
