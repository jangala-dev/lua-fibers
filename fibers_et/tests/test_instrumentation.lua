...
local rt =
  Runtime.new({ instrumentation = { trace = true, trace_limit = 32, slow_plan_limit = 4 } })
...
eq(
  pruned_snap.maxima.search_steps_per_plan,
  1,
  'irrelevant recruitment should be pruned at the root'
)
...
truthy(
  (pruned_snap.counters.component_roots_excluded or 0) > 0,
  'component isolation was not exercised'
)
...
local legacy_pruned =
  Runtime.new({ machine = 'trail', component_search = false, instrumentation = true })
for i = 1, 5 do
...
truthy(
  (legacy_snap.counters.footprint_checks or 0) > 0,
  'legacy footprint pruning was not exercised'
)
