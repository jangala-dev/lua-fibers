...
local got
rt:spawn_raw(function() got = rt:perform(ch:get_op()) end, 'instrumented-get')
rt:spawn_raw(function() rt:perform(ch:put_op('ok')) end, 'instrumented-put')
local status = rt:run()
eq(status.tag, 'found')
eq(got, 'ok')
...
for i = 1, 20 do
  ...
  pruned:spawn_raw(function() pruned:perform(unrelated:get_op()) end, 'unrelated-get-' .. tostring(i))
end
...
for i = 1, 5 do
  ...
  legacy_pruned:spawn_raw(function() legacy_pruned:perform(unrelated:get_op()) end, 'legacy-unrelated-' .. tostring(i))
end
