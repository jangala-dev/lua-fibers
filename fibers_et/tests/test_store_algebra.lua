...
local S = require('fibers.kernel.store')

local function fail(msg) error(msg, 2) end
local function eq(a,b,msg) if a~=b then fail((msg or 'not equal')..': '..tostring(a)..' ~= '..tostring(b)) end end
local function truth(v,msg) if not v then fail(msg or 'expected true') end end
local function map_eq(a,b,msg)
  for k,v in pairs(a or {}) do if b[k]~=v then fail((msg or 'map mismatch')..' at '..tostring(k)) end end
  for k,v in pairs(b or {}) do if a[k]~=v then fail((msg or 'map mismatch')..' at '..tostring(k)) end end
end

local add = S.new_location({merge='add', value=0})
local a={kind='add',delta=2}; local b={kind='add',delta=-1}; local c={kind='add',delta=4}
local ab=S.merge_parallel(add,a,b,'independent'); local ba=S.merge_parallel(add,b,a,'independent')
eq(ab.delta,ba.delta,'add parallel composition should commute')
local abc1=S.merge_parallel(add,ab,c,'independent')
local bc=S.merge_parallel(add,b,c,'independent'); local abc2=S.merge_parallel(add,a,bc,'independent')
...
map_eq(S.apply_patch_value(fm,{},xy),S.apply_patch_value(fm,{},yx),'disjoint finite-map edits should commute')
local down=S.constraint_projection(fm,{kind='finite_map',ops={{op='put',key='x',value=1},{op='remove',key='z'}}},'up')
eq(#down.ops,1); eq(down.ops[1].op,'remove','upward claims retain only downward constraints')
local up=S.constraint_projection(fm,{kind='finite_map',ops={{op='put',key='x',value=1},{op='remove',key='z'}}},'down')
eq(#up.ops,1); eq(up.ops[1].op,'put','downward claims retain only upward constraints')
local hand=S.merge_parallel(fm,px,{kind='finite_map',ops={{op='take',key='x'}}},'interacting')
