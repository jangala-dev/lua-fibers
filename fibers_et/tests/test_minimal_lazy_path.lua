...
-- Immutable common operations are constructed once per resource.  This is an
...
do
  ...
  eq(rendezvous:get_op(), rendezvous:get_op(), 'rendezvous get operation should be cached')

  local scalar = Scalar.new(0, 'minimal-cached-scalar')
  eq(scalar:read_op(), scalar:read_op(), 'scalar read operation should be cached')
  eq(scalar:snapshot_op(), scalar:snapshot_op(), 'scalar snapshot operation should be cached')
  truthy(
  ...
  eq(counter:read_op(), counter:read_op(), 'counter read operation should be cached')
  eq(counter:state_op(), counter:state_op(), 'counter state operation should be cached')

  local queue = EventQueue.new('minimal-cached-events')
  eq(queue:next_op(), queue:next_op(), 'event queue next operation should be cached')
  eq(queue:_drain_op(), queue:_drain_op(), 'event queue drain operation should be cached')

  local readiness = Readiness.new(1, 'read', 'minimal-cached-readiness')
  eq(readiness:readable_op(), readiness:readable_op(), 'read readiness operation should be cached')
  eq(readiness:writable_op(), readiness:writable_op(), 'write readiness operation should be cached')

  local signal = Signal.new('minimal-cached-signal')
  eq(signal:wait_op(), signal:wait_op(), 'signal wait operation should be cached')
end
