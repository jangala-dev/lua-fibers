...
local Op, Petri = fibers.Op, fibers.Petri
...
do
  local net = Petri.new({ jobs = { { id = 'a', priority = 2 }, { id = 'b', priority = 9 } }, workers = { 'w1' } })
  local start = net:transition({
    inputs = {
      { place = 'jobs', as = 'job', where = function(job) return job.priority >= 5 end },
      { place = 'workers', as = 'worker' },
    },
    produce = function(b)
      return { { place = 'running', value = { job = b.job.id, worker = b.worker } } }
    end,
    result = function(b) return b.job.id, b.worker end,
  })
  local job, worker
  run(function() job, worker = fibers.perform(net:fire_op(start)) end)
  assert(job == 'b' and worker == 'w1')
  local m = net:snapshot()
...
do
  local net = Petri.new()
  local rows
  run(function() rows = fibers.perform(Op.tensor({ net:put_op('p', 'x'), net:take_op('p') })) end)
...
do
  ...
  local any = net:transition({ inputs = { { place = 'p', as = 'x' } }, result = function(b) return b.x end })
  local red = net:transition({
    inputs = { { place = 'p', as = 'x', where = function(x) return x == 'red' end } },
    result = function(b) return b.x end,
  })
  local rows
  run(function() rows = fibers.perform(Op.all({ net:fire_op(any), net:fire_op(red) })) end)
...
do
  ...
  local add = net:transition({
    ...
    produce = function(b) return { sum = { b.a + b.b } } end,
    result = function(b) return b.a + b.b end,
  })
  local value
  run(function() value = fibers.perform(net:fire_op(add)) end)
  assert(value == 5)
  local m = net:snapshot()
...
do
  ...
  local impossible = net:transition({
    inputs = { { place = 'p', as = 'x', where = function(x) return x > 10 end } },
  })
  local result
  run(function() result = fibers.perform(net:fire_op(impossible):or_else(Op.always('none'))) end)
