...
do
  ...
  local r = fibers.try_run(function()
    local never = fibers.Signal.new('structured-never')
    ...
    body_cancelled = not ok and fibers.Runtime.is_cancelled(err)
  end)
  ...
  local state
  fibers.run(function() state = fibers.perform(sibling:state_op()) end)
...
do
  ...
  local r = fibers.try_run(function()
    return fibers.try_scope({ policy = fibers.policy.supervisor({ child_failure = 'fail_at_exit' }) }, function()
      local ready = fibers.Rendezvous.new('supervisor-ready')
      fibers.spawn(function() error('supervised boom', 0) end, 'supervised-failure')
      fibers.spawn(function()
...
do
  local r = fibers.try_run(function()
    return fibers.try_scope({ policy = fibers.policy.supervisor({ child_failure = 'collect' }) }, function()
      fibers.spawn(function() error('collected boom', 0) end)
      return 42
    end)
  end)
  assert_truthy(r.ok)
  ...
  assert_truthy(inner.report and #inner.report.child_failures == 1, 'collected failure should remain in the report')
  assert_truthy(tostring(inner.report):match('completed with 1 child failure') ~= nil, 'successful collection report should not claim the scope failed')
end
...
do
  ...
  fibers.run(function()
    local ok, err = pcall(function() fibers.spawn_raw(function() end) end)
    denied = not ok and tostring(err):match('prohibited') ~= nil
  end)
  fibers.run(function()
    fibers.spawn_raw(function() allowed = true end)
  end, { policy = fibers.policy.nursery({ allow_unstructured = true }) })
...
do
  ...
  local r = fibers.try_run(function()
    local never = fibers.Signal.new('nested-cancel-never')
    ...
    fibers.spawn(function() error('parent failure', 0) end, 'parent-failure')
  end)
  assert_eq(r.ok, false)
  assert_eq(r.reason, 'child_failed')
...
do
  ...
  fibers.run(function(root)
    local h = fibers.Region.handle('strict-move')
    fibers.scope({ policy = fibers.policy.nursery({ allow_outward_move = false }) }, function(inner)
      fibers.perform(inner:admit_op(h))
      local ok, err = pcall(function() inner:move_op(h, root) end)
...
do
  ...
  local r = fibers.try_run(function(scope)
    local never = fibers.Signal.new('late-admission-never')
    fibers.spawn(function() error('close before late admission', 0) end, 'closing-child')
    local ok, err = fibers.pcall(function()
      fibers.perform(never:wait_op())
    end)
    assert_truthy(not ok and fibers.Runtime.is_cancelled(err), 'body should observe fail-fast cancellation')
    fibers.mask(function()
      late_result = fibers.perform(
        scope:spawn_op(function() late_started = true end, 'too-late')
          :map(function() return 'admitted' end)
...
do
  local r = fibers.try_run(function()
    return fibers.try_scope({ policy = fibers.policy.supervisor({ child_failure = 'collect' }) }, function()
      fibers.spawn(function() error('first collected failure', 0) end, 'first-collected')
      fibers.spawn(function() error('second collected failure', 0) end, 'second-collected')
      return 'done'
    end)
  end)
  assert_truthy(r.ok)
