...
-- Observation also protects external resource observations if a producer bypasses the runtime-wide epoch.
do
...
do
  ...
  local resource = {
    ...
    _fibers_external_clear = function(self) self.value = nil end,
  }
  ...
  local ok = pcall(function() other:deliver(feed, 'wrong-runtime') end)
...
do
  ...
  local rt = Runtime.new()
  assert_eq(rt:external_feed(signal), rt:external_feed(signal), 'feed capability should be cached per runtime/resource')
end
