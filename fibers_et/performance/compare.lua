...
if not baseline_path or not candidate_path then
  io.stderr:write('usage: lua performance/compare.lua BASELINE.csv CANDIDATE.csv [TIME_PERCENT] [SEARCH_PERCENT]\n')
  os.exit(2)
end
...
local function parse_line(line)
  ...
  while i <= #line do
    ...
    if quoted then
      if ch == '"' and line:sub(i + 1, i + 1) == '"' then field = field .. '"'; i = i + 1
      elseif ch == '"' then quoted = false
      else field = field .. ch end
    elseif ch == '"' then quoted = true
    elseif ch == ',' then out[#out + 1] = field; field = ''
    else field = field .. ch end
    i = i + 1
  end
  out[#out + 1] = field
...
local function load(path)
  ...
  local index = {}
  for i = 1, #header do index[header[i]] = i end
...
local function percent(old, new)
  if old == nil or new == nil then return nil end
  if old == 0 then return new == 0 and 0 or math.huge end
  return (new / old - 1) * 100
end

local baseline, candidate = load(baseline_path), load(candidate_path)
local failed = false
print(string.format('%-55s %11s %11s %9s %9s %9s %9s',
  'case', 'base us/op', 'new us/op', 'time', 'base p99<=', 'new p99<=', 'search'))
...
for _, key in ipairs(keys) do
  ...
  if old and old.us and new.us then
    ...
    if time_bad or search_bad then failed = true end
    local marker = (time_bad and 'TIME ' or '') .. (search_bad and 'SEARCH' or '')
    print(string.format('%-55s %11.3f %11.3f %+8.1f%% %9s %9s %8s %s',
      key, old.us, new.us, time_change,
      tostring(old.p99_steps or ''), tostring(new.p99_steps or ''),
      search_change and string.format('%+.1f%%', search_change) or '', marker))
  else
