...
local function env_number(name, default) return tonumber(env(name, '')) or default end
...
local machines = {}
for value in machines_text:gmatch('[^,%s]+') do machines[#machines + 1] = value end
...
local function copy_options(base, additions)
  ...
  for k, v in pairs(additions or {}) do out[k] = v end
  return out
end
...
local function runtime(profile, machine, extra)
  return Runtime.new(copy_options(profile.options, copy_options(extra, {
    machine = machine,
...
local scenarios = {}
local function add(name, run) scenarios[#scenarios + 1] = { name = name, run = run } end
...
add('isolated blocked components', function(profile, machine)
  ...
  for i = 1, 24 do
    ...
    rt:spawn_raw(function() rt:perform(channel:get_op()) end)
  end
...
add('hinted continuation isolation', function(profile, machine)
  local rt = runtime(profile, machine, { dependency_index_threshold = 1, dependency_index_release_threshold = 0 })
  local focus = Rendezvous.new('arch-hinted-focus')
  rt:spawn_raw(function()
    rt:perform(Op.guard(function() return focus:get_op() end, Op.dependencies(focus:get_op())))
  end)
  for i = 1, 12 do
...
add('opaque continuation slow path', function(profile, machine)
  ...
  rt:spawn_raw(function() rt:perform(Op.guard(function() return focus:get_op() end)) end)
  for i = 1, 12 do
...
add('binary rendezvous sequence', function(profile, machine)
  ...
  local total, n = 0, 80
  rt:spawn_raw(function()
    for _ = 1, n do total = total + rt:perform(channel:get_op()) end
  end)
  rt:spawn_raw(function()
    for i = 1, n do rt:perform(channel:put_op(i)) end
  end)
  local status = drain(rt)
...
add('forced scalar query sequence', function(profile, machine)
  ...
  local total, n = 0, 80
  rt:spawn_raw(function()
    for _ = 1, n do if rt:perform(scalar:expect_op(9)) then total = total + 1 end end
  end)
  local status = drain(rt)
  assert(status.tag == 'idle' or status.tag == 'quiescent')
  assert(total == n)
...
add('contended producers', function(profile, machine)
  ...
  for producer = 1, producers do
    rt:spawn_raw(function()
      for message = 1, messages do rt:perform(channel:put_op(producer * 100 + message)) end
    end)
  end
  rt:spawn_raw(function()
    for _ = 1, producers * messages do total = total + rt:perform(channel:get_op()) end
  end)
  local status = drain(rt)
  assert(status.tag == 'idle' or status.tag == 'quiescent')
  return rt, 'sum:' .. tostring(total)
...
add('triple swap with decoy', function(profile, machine)
  ...
  local a, b, c
  rt:spawn_raw(function() a = rt:perform(Op.all({ ab:put_op('A'), ca:get_op() }):map(function(rows) return rows[2][1] end)) end)
  rt:spawn_raw(function() b = rt:perform(Op.all({ bc:put_op('B'), ab:get_op() }):map(function(rows) return rows[2][1] end)) end)
  rt:spawn_raw(function() c = rt:perform(Op.all({ ca:put_op('C'), bc:get_op() }):map(function(rows) return rows[2][1] end)) end)
  rt:spawn_raw(function() rt:perform(ab:get_op()) end)
  drain(rt)
...
local function nursery_case(fanout)
  return function(profile, machine)
    ...
    local result = fibers.try_run(function()
      ...
      for i = 1, fanout do fibers.spawn(function() fibers.perform(channel:put_op(i)) end, 'arch-child-' .. tostring(i)) end
      for _ = 1, fanout do total = total + fibers.perform(channel:get_op()) end
    end, opts)
...
if include_fanout8 then add('nursery rendezvous fanout eight', nursery_case(8)) end
    ...
    for _, profile in ipairs(profiles) do
      for _, machine in ipairs(machines) do
        ...
        for _ = 1, repeats do
          ...
          if digest ~= nil then assert(digest == value, 'non-replayable scenario digest') end
          digest = value
        end
        local key = scenario.name
        if digests[key] ~= nil then assert(digests[key] == digest, 'semantic digest differs for ' .. key) else digests[key] = digest end
        local c, m = last_snapshot.counters or {}, last_snapshot.maxima or {}
        rows[#rows + 1] = {
          case = scenario.name, profile = profile.name, machine = machine,
          elapsed = median(elapsed_samples), digest = digest,
          plans = c.plans or 0, search_calls = c.search_calls or 0,
          branches = c.branches or 0, max_steps = m.search_steps_per_plan or 0,
          component_fraction = (c.frontier_roots_total or 0) > 0 and (c.component_roots_total or 0) / c.frontier_roots_total or 1,
          excluded = c.component_roots_excluded or 0,
          forced_exchanges = c.forced_exchanges or 0, forced_claims = c.forced_claims or 0,
          dynamic = c.requests_dynamic or 0, analysable = c.requests_analysable or 0,
          states = c.states_observed or 0, duplicates = c.state_duplicates or 0,
          pair_scans = c.intent_pairs_scanned or 0, footprint_checks = c.footprint_checks or 0,
        }
      end
    end
  end
...
local function render_csv()
  local lines = { 'case,profile,machine,median_seconds,digest,plans,search_calls,branches,max_search_steps,component_fraction,component_roots_excluded,forced_exchanges,forced_claims,requests_analysable,requests_dynamic,states_observed,state_duplicates,intent_pairs_scanned,footprint_checks' }
  for _, r in ipairs(rows) do
    local values = { r.case, r.profile, r.machine, string.format('%.9f', r.elapsed), r.digest,
      r.plans, r.search_calls, r.branches, r.max_steps, string.format('%.6f', r.component_fraction),
      r.excluded, r.forced_exchanges, r.forced_claims, r.analysable, r.dynamic,
      r.states, r.duplicates, r.pair_scans, r.footprint_checks }
    for i = 1, #values do values[i] = quote(values[i]) end
...
local function render_text()
  local lines = {
    ...
    string.format('lua=%s repeats=%d machines=%s clock=%s', tostring(_VERSION), repeats, machines_text, Clock.name),
    string.format('%-34s %-12s %-9s %9s %9s %9s %8s %7s %7s %7s',
      'case', 'profile', 'machine', 'median ms', 'max step', 'branches', 'comp%', 'forced', 'dynamic', 'dupes'),
    string.rep('-', 124),
  }
  for _, r in ipairs(rows) do
    lines[#lines + 1] = string.format('%-34s %-12s %-9s %9.3f %9d %9d %7.1f%% %7d %7d %7d',
      r.case, r.profile, r.machine, r.elapsed * 1000, r.max_steps, r.branches,
      r.component_fraction * 100, r.forced_exchanges + r.forced_claims, r.dynamic, r.duplicates)
  end
  lines[#lines + 1] = ''
...
io.write(rendered)
if output ~= '' then local f = assert(io.open(output, 'wb')); f:write(rendered); f:close() end
