...
package.path = table.concat({
  join_path(root, '?.lua'), join_path(root, '?/init.lua'), join_path(root, '?/?.lua'),
  './?.lua', './?/init.lua', './?/?.lua', '../?.lua', '../?/?.lua', package.path,
}, ';')
...
for tier in string.gmatch(tiers_text, '[^,%s]+') do selected_tiers[tier] = true end
if tiers_text == 'all' then selected_tiers = { simple = true, moderate = true, complex = true } end
...
local function min_value(values)
  local out = values[1]
  for i = 2, #values do if values[i] < out then out = values[i] end end
  return out
end
...
local function bucket_upper(label)
  if label == nil or label == 'nil' then return 0 end
...
local function histogram_quantile_upper(histogram, quantile)
  if type(histogram) ~= 'table' then return 0 end
  ...
  table.sort(buckets, function(a, b) return a.upper < b.upper end)
  local target, seen = math.max(1, math.ceil(total * quantile)), 0
  for i = 1, #buckets do
    seen = seen + buckets[i].count
    if seen >= target then return buckets[i].upper end
  end
  return buckets[#buckets].upper
end

local function case_matches(case)
  if not selected_tiers[case.tier] then return false end
  if case_filter == '' then return true end
...
function Context:instrumentation_options()
  if not self.instrumented then return nil end
  return { trace = trace, trace_limit = 512, slow_plan_limit = slow_plan_limit, clock = Clock.now, state_hash = state_hash }
end

function Context:runtime(opts)
  opts = opts or {}
  local rt_opts = {}
  for k, v in pairs(opts) do rt_opts[k] = v end
...
function Context:add_runtime(rt)
  ...
  for i = 1, #self.runtimes do if self.runtimes[i] == rt then return end end
  self.runtimes[#self.runtimes + 1] = rt
end
...
local function merge_snapshot(dst, src)
  ...
  for k, v in pairs(src.counters or {}) do dst.counters[k] = (dst.counters[k] or 0) + v end
  for k, v in pairs(src.maxima or {}) do
    if dst.maxima[k] == nil or v > dst.maxima[k] then dst.maxima[k] = v end
  end
  for name, histogram in pairs(src.histograms or {}) do
    local target = dst.histograms[name]
    if not target then target = {}; dst.histograms[name] = target end
    for bucket, count in pairs(histogram) do target[bucket] = (target[bucket] or 0) + count end
  end
  for i = 1, #(src.slow_plans or {}) do dst.slow_plans[#dst.slow_plans + 1] = src.slow_plans[i] end
  table.sort(dst.slow_plans, function(a, b)
    if (a.search_steps or 0) ~= (b.search_steps or 0) then return (a.search_steps or 0) > (b.search_steps or 0) end
    return (a.elapsed or 0) > (b.elapsed or 0)
  end)
  while #dst.slow_plans > slow_plan_limit do dst.slow_plans[#dst.slow_plans] = nil end
  return dst
end

function Context:snapshot()
  local out
  for i = 1, #self.runtimes do out = merge_snapshot(out, self.runtimes[i]:instrumentation_snapshot()) end
...
for _, case in ipairs(cases) do
  if case_matches(case) then
    ...
    if warmup and n >= 20 then run_once(case, math.max(1, math.floor(n / 20)), false) end
    ...
    results[#results + 1] = {
      ...
      diagnostics = diag,
      search_calls_per_plan = (counters.plans or 0) > 0 and (counters.search_calls or 0) / counters.plans or 0,
      ...
      p95_search_cpu_us_upper = histogram_quantile_upper(diag and diag.histograms.search_cpu_us_per_plan, 0.95),
      p99_search_cpu_us_upper = histogram_quantile_upper(diag and diag.histograms.search_cpu_us_per_plan, 0.99),
      ...
      component_fraction = (counters.frontier_roots_total or 0) > 0
        and (counters.component_roots_total or 0) / counters.frontier_roots_total or 1,
      state_duplicate_fraction = (counters.states_observed or 0) > 0
        and (counters.state_duplicates or 0) / counters.states_observed or 0,
      forced_exchanges = counters.forced_exchanges or 0,
      forced_claims = counters.forced_claims or 0,
      dynamic_request_fraction = ((counters.requests_dynamic or 0) + (counters.requests_analysable or 0)) > 0
        and (counters.requests_dynamic or 0) / ((counters.requests_dynamic or 0) + (counters.requests_analysable or 0)) or 0,
    }
  end
end

if #results == 0 then error('no performance cases matched the selected tiers and filter', 0) end
...
local function json_encode(value, indent)
  ...
  if is_array(value) then
    ...
    local rows = {}
    for i = 1, #value do rows[i] = child_pad .. json_encode(value[i], indent + 2) end
    return '[\n' .. table.concat(rows, ',\n') .. '\n' .. pad .. ']'
  end
...
local function render_text()
  ...
  lines[#lines + 1] = string.format('lua=%s machine=%s seed=%d scale=%s repeats=%d tiers=%s diagnostics=%s clock=%s',
    tostring(_VERSION), machine, choice_seed, tostring(scale), repeats, tiers_text, tostring(diagnostics), Clock.name)
  lines[#lines + 1] = string.format('%-9s %-13s %-35s %10s %11s %11s %9s %10s %8s %7s %7s',
    'tier', 'group', 'case', 'ops', 'median ms', 'us/op', 'p99<=', 'max steps', 'branches', 'comp%', 'dyn%')
  lines[#lines + 1] = string.rep('-', 147)
  for _, r in ipairs(results) do
    lines[#lines + 1] = string.format('%-9s %-13s %-35s %10d %11.3f %11.3f %9d %10d %8.1f %6.1f%% %6.1f%%',
      r.tier, r.group, r.name, r.operations, r.median_seconds * 1000,
      r.median_us_per_op, r.p99_search_steps_upper, r.max_search_steps, r.branches_per_plan,
      r.component_fraction * 100, r.dynamic_request_fraction * 100)
  end
  lines[#lines + 1] = ''
...
local function render_csv()
  local lines = { 'tier,group,name,iterations,operations,median_seconds,min_seconds,max_seconds,median_us_per_op,median_retained_kb,diagnostic_seconds,plans,search_calls,search_calls_per_plan,branches_per_plan,p50_search_steps_upper,p95_search_steps_upper,p99_search_steps_upper,p95_search_cpu_us_upper,p99_search_cpu_us_upper,max_search_steps,max_search_depth,max_pending,claim_branches,recruit_branches,footprint_checks,footprint_matches,component_fraction,state_duplicate_fraction,forced_exchanges,forced_claims,requests_analysable,requests_dynamic,dynamic_request_fraction,component_roots_excluded,plan_reuse_eligible' }
  for _, r in ipairs(results) do
    local c = r.diagnostics and r.diagnostics.counters or {}
    local row = {
      r.tier, r.group, r.name, r.iterations, r.operations,
      string.format('%.9f', r.median_seconds), string.format('%.9f', r.min_seconds), string.format('%.9f', r.max_seconds),
      string.format('%.6f', r.median_us_per_op), string.format('%.3f', r.median_retained_kb),
      r.diagnostic_seconds and string.format('%.9f', r.diagnostic_seconds) or '',
      c.plans or 0, c.search_calls or 0, string.format('%.3f', r.search_calls_per_plan),
      string.format('%.3f', r.branches_per_plan), r.p50_search_steps_upper, r.p95_search_steps_upper,
      r.p99_search_steps_upper, r.p95_search_cpu_us_upper, r.p99_search_cpu_us_upper,
      r.max_search_steps, r.max_search_depth, r.max_pending, c.claim_branches or 0,
      c.recruit_branches or 0, c.footprint_checks or 0, c.footprint_matches or 0,
      string.format('%.6f', r.component_fraction), string.format('%.6f', r.state_duplicate_fraction),
      r.forced_exchanges, r.forced_claims, c.requests_analysable or 0, c.requests_dynamic or 0,
      string.format('%.6f', r.dynamic_request_fraction), c.component_roots_excluded or 0,
      c.plan_reuse_eligible or 0,
    }
    for i = 1, #row do row[i] = csv_quote(row[i]) end
...
local rendered
if format == 'json' then rendered = json_encode(document) .. '\n'
elseif format == 'csv' then rendered = render_csv()
else rendered = render_text() end

io.write(rendered)
if output_path ~= '' then
  local file, err = io.open(output_path, 'wb')
  if not file then error('cannot write performance output: ' .. tostring(err), 0) end
  file:write(rendered)
  file:close()
end

if not rawget(_G, '_FIBERS_PERF_EMBEDDED') and os and os.exit then os.exit(0, false) end
return document
