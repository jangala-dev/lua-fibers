...
It does not support polling or arbitrary host events.  More capable Linux hosts
can provide nanosleep, epoll, select, nixio, libuv, or another event mechanism
without changing `sleep_op` itself.

## Observation journal

Clock waits participate in the observation journal.  A bounded cursor that has
