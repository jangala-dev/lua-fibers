...
```text
source    host/manual/readiness occurrence
...
clock, a manual callback source, or a readiness source.

```lua
local clock = fibers.Source.clock('clock')
local signal = fibers.Source.manual('signal')
local fd_read = fibers.Source.poll(fd, 'read')
```

Sources bring outside facts into operations.  Effects publish committed
obligations outwards.
...
A host may expose readiness through:

```lua
fibers.Runtime.new({
  host = {
    source_ready = function(key, mode, source, rt)
      return false
    end,
  },
})
...
or tests and simple adapters may set readiness on the source object itself.
