...
local function clone_state(s)
  ...
  for id, g in pairs(s.groups) do
    local lane_outcomes = {}
    for i = 1, g.count do lane_outcomes[i] = g.lane_outcomes[i] end
    out.groups[id] = {
      id = g.id,
...
local function new_view(state, root_id, scope_path, source_view_id)
  ...
  state.views[id] = Store.new_view(root_id, copy_scope_path(scope_path), source)
  return id
end
...
local function product_wrap(lane_outcomes)
  ...
  for i = 1, #lane_outcomes do
    if lane_outcomes[i] and lane_outcomes[i].wrap then has_wrap = true; break end
  end
  ...
  return function(packed)
    ...
    for i = 1, #lane_outcomes do
      ...
      if lane.wrap then rows[i] = lane.wrap(rows[i]) end
    end
    return pack_(rows)
  end
end
  ...
  while true do
    ...
    if frame.kind == 'bind' then
      ...
      elseif frame.phase == 'map' then
        task.expr = Op.always(state.runtime:_call_in_phase('map', 'callback_error', frame.fn, unpack_pack(outcome.pack)))
      else
...
local function result_pack(program, value)
  ...
  if kind == 'presence_bool' then return pack_(value ~= Store.ABSENT) end
  if kind == 'presence_value' then
    if value == Store.ABSENT then return pack_(nil) end
    if program.nil_sentinel and value == program.nil_sentinel then return pack_(nil) end
    return pack_(value)
  end
...
local function predicate_holds(program, value)
  if program.predicate == 'present' then return value ~= Store.ABSENT end
  if program.predicate == 'absent' then return value == Store.ABSENT end
...
local function select_extreme(program, value)
  ...
  if program.order == 'max' then return candidates[#candidates] end
  return candidates[1]
end
...
local function evaluate_claim(program, value)
  ...
  if not query and program.kind == 'conditional_claim' then
    query = {
      kind = 'predicate', predicate = program.predicate,
      threshold = program.threshold, key = program.key,
    }
  end
  ...
  if query.kind == 'predicate' then
    ...
    if not predicate_holds(probe, value) then return nil end
  ...
  if query.kind == 'extreme' then result_value = witness.entry end
  return {
    patch = patch,
...
local function resolve_claims(state, intent_ids)
  ...
  for i = 1, #selected do
    ...
    if resolution.patch then Store.stage(state.views[task.view_id], loc, resolution.patch) end
    resolved[#resolved + 1] = {
      intent = intent,
      task = task,
...
local function execute_program(state, task, program)
  ...
  if program.kind == 'patch' then
    Store.stage(view, loc, program.patch)
    return complete_task(state, task, { pack = result_pack(program, Store.read(view, loc)) })
  end
    ...
    if #state.active > 0 then
      ...
      if task.status ~= 'active' then
        ...
        elseif kind == 'annotated' then
          if expr.post then task.frames[#task.frames + 1] = { kind = 'wrap', fn = expr.post } end
          task.expr = expr.p
          add_active(state, task.id)
        ...
        elseif kind == 'choice' then
          ...
          local order = ChoiceOrder.indices(state.runtime, task, task.choice_serial, #(expr.choices or {}))
          for k = 1, #order do
            local i = order[k]
            ...
            if found then
              ...
              for j = 1, #(expr.choices or {}) do
                if j ~= i then collect_defeat_effects(expr.choices[j], defeats) end
              end
        ...
        elseif kind == 'or_else' then
          ...
          for i = 1, #((pref and pref.checks) or {}) do fallback.negative_checks[#fallback.negative_checks + 1] = pref.checks[i] end
          for i = 1, #((pref and pref.interests) or {}) do fallback.fallback_interests[#fallback.fallback_interests + 1] = pref.interests[i] end
      ...
      for gi = 1, #groups do
        ...
        local all_machine, machine_supply_none = true, true
        for ii = 1, #group.ids do
          local found_intent
          for jj = 1, #state.intents do
            if state.intents[jj].id == group.ids[ii] then found_intent = state.intents[jj]; break end
          end
          if not found_intent or found_intent.kind ~= 'machine_transition' then
            all_machine, machine_supply_none = false, false
            break
          end
          if found_intent.program.transition.supply ~= 'none' then machine_supply_none = false end
        end
