...
function Rope:prepend(s)
  ...
  -- Returning a lease is not the common hot path.  Keep the simple, safe path:
  -- insert a chunk before the current head without materialising the tail.
  if self.head_idx > 1 then
    self.head_idx = self.head_idx - 1
    self.chunks[self.head_idx] = s
  else
    table.insert(self.chunks, 1, s)
    self.head_idx = 1
  end
  self.len = self.len + #s
end
