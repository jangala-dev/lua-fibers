...
local function with_mask(frame, fn)
  ...
  local ok, a, b, c, d = Protected.pcall(fn)
...
function NurseryFrame:_cancel_owned(reason)
  for i = 1, #self.children do
    ...
    if self:_owns(task) then with_mask(self, function() self:perform(self.lifetime:cancel_op(task, reason)) end) end
  end
end
...
function NurseryFrame:_join_and_settle_owned()
  ...
  for i = 1, #self.children do
    ...
    if self:_owns(task) then
      local status, value, report = with_mask(self, function() return self:perform(task:join_op()) end)
      if status == 'failed' and not first_bad then first_bad = report or { status = status, value = value } end
      if self:_owns(task) then with_mask(self, function() self:perform(self.lifetime:release_op(task)) end) end
    end
  end
  return first_bad
end
...
function Nursery:run_root(frame, fn)
  ...
  with_mask(frame, function() frame:perform(frame.lifetime:seal_op()) end)
  ...
  if child_bad then error(child_bad.error or child_bad.reason or child_bad.value or 'nursery child failed', 0) end
