...
local function copy_tokens(tokens)
  local out = {}; for i = 1, #(tokens or {}) do out[i] = tokens[i] end; return out
end

local function materialise_places(s)
  local out = {}
  for place in pairs(s.place_names or {}) do out[place] = s.places[place] or {} end
  return out
end

local function clone_state(s)
  local parent, depth = s.places, (s.depth or 0) + 1
  if depth > 16 then parent, depth = materialise_places(s), 0 end
  return { next_token = s.next_token or 0, places = setmetatable({}, { __index = parent }),
    local_places = {}, place_names = s.place_names, depth = depth }
end
...
local function ensure_place(state, place)
  ...
  if not state.place_names[place] then
    local names = {}; for k in pairs(state.place_names) do names[k] = true end
    names[place] = true; state.place_names = names
  end
...
local function snapshot_state(s)
  ...
  for place in pairs(s.place_names or {}) do
    ...
    for i = 1, #tokens do ys[i] = tokens[i].value end
    out[place] = ys
  end
  return out
end
...
local function remove_token_ids(state, consumed)
  for place, ids in pairs(consumed) do
    ...
    for i = 1, #source do local token = source[i]; if not ids[token.id] then keep[#keep + 1] = token end end
    state.local_places[place] = true; rawset(state.places, place, keep)
  end
end

local function normalise_produced(produced)
  local out = {}
  if not produced then return out end
  ...
  for place, values in pairs(produced) do
    ...
    elseif type(values) == 'table' then
      for i = 1, #values do out[#out + 1] = { place = place, value = values[i] } end
    else
      out[#out + 1] = { place = place, value = values }
    end
  end
  table.sort(out, function(a, b) return tostring(a.place) < tostring(b.place) end)
  return out
end
...
function Petri.new(marking, name)
  ...
  for place, values in pairs(marking or {}) do
    ...
    for i = 1, #values do add_token(state, place, values[i]) end
  end
...
local function binding_cursor(transition, state, payload)
  local co = coroutine.create(function()
    ...
    local function emit()
      if transition.guard and not transition.guard(bindings, payload, state) then return end
      local successor, consumed = clone_state(state), {}
      for i = 1, #chosen do
        local c = chosen[i]
        if c.consume then
          local ids = consumed[c.place] or {}; consumed[c.place] = ids; ids[c.token.id] = true
        end
      end
      remove_token_ids(successor, consumed)
      local produced = transition.produce
      if type(produced) == 'function' then produced = produced(bindings, payload, state) end
      local xs = normalise_produced(produced)
      for i = 1, #xs do add_token(successor, xs[i].place, xs[i].value) end
      local result
      if transition.result then result = Op._pack(transition.result(bindings, payload, successor))
      else local copy = {}; for k, v in pairs(bindings) do copy[k] = v end; result = Op._pack(copy) end
      coroutine.yield({ value = successor, result = result, writes = true })
    end
    local function search(i)
      if i > #inputs then emit(); return end
      ...
      for ti = 1, #tokens do
        ...
        if not consume or not used[token.id] then
          ...
          if ok then
            if consume then used[token.id] = true end
            local old, had = bindings[arc.as], arc.as ~= nil and bindings[arc.as] ~= nil
            if arc.as ~= nil then bindings[arc.as] = token.value end
            chosen[#chosen + 1] = { place = arc.place, token = token, consume = consume }
            search(i + 1)
            chosen[#chosen] = nil
            if arc.as ~= nil then bindings[arc.as] = had and old or nil end
            if consume then used[token.id] = nil end
          end
        end
      end
    end
    if #inputs == 0 then emit() else search(1) end
  end)
...
function Petri:take_op(place, predicate)
  local t = self:transition({
    ...
    inputs = { { place = place, as = 'value', where = predicate and function(v) return predicate(v) end or nil } },
    result = function(b) return b.value end,
  })
  return self:fire_op(t)
end

function Petri:marking_op()
  return Op._resource(self, Kind, Program.witness_transition({
    location = self._location,
    supply = 'none',
    cursor = function(state)
      local done = false
      return { next = function() if done then return nil end; done = true; return { writes = false, result = Op._pack(snapshot_state(state)) } end }
    end,
  }))
end
