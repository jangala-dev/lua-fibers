...
local function make_unsupported(prefix, reason)
  return {
    ...
    wrap = function() error(prefix .. ': ' .. tostring(reason), 2) end,
    pipe = function() error(prefix .. ': ' .. tostring(reason), 2) end,
  }
end
...
function Common.new(opts)
  ...
  local function set_nonblocking_fd(fd, value)
    local flags, e = retrying_syscall(function() return C.fcntl(fd, F_GETFL, 0) end)
    if not flags then return nil, strerror(e), e end
    local new_flags
    if value ~= false then
    ...
    local ok, e2 = retrying_syscall(function() return C.fcntl(fd, F_SETFL, new_flags) end)
    if not ok then return nil, strerror(e2), e2 end
    return true
  end
  ...
  local function fd_read(self, max)
    ...
    while true do
      ...
      if n == 0 then return nil, Errors.EOF end
      local e = errno()
      if e == EINTR then
        -- retry
  ...
  local function fd_shutdown_read(self, _reason)
    ...
    pcall(function() C.shutdown(self.fd, SHUT_RD) end)
    return true
  end

  local function fd_shutdown_write(self, _reason)
    pcall(function() C.shutdown(self.fd, SHUT_WR) end)
    return true
  end
  ...
  local function fd_close(self, _reason)
    ...
    while true do
      ...
      local e = errno()
      if e ~= EINTR then return nil, strerror(e), e end
    end
  end
  ...
  function Fd.wrap(fd, wrap_opts)
    ...
    h.raw_fd = fd
    if wrap_opts.nonblocking ~= false then h:set_nonblocking(true) end
    return h
  end
