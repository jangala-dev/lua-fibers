-- Linux epoll host adapter using the cffi module.
--
-- This mirrors the LuaJIT FFI backend, but deliberately requires cffi rather
-- than falling back to LuaJIT ffi.  It is optional and should skip cleanly when
-- cffi is unavailable.
...
local function unsupported(reason)
  return {
    ...
    new = function() error('fibers.host.cffi_linux: ' .. tostring(reason), 2) end,
  }
end
...
if not bit then
  ...
  if ok_bit32 then bit = bit32_mod end
end
if not bit then
...
-- cffi may return numeric cdata where LuaJIT FFI returns Lua numbers.
-- Normalise all C numeric boundary values through ffi.tonumber.

local function tonumber_c(v)
  local n = toint(v)
...
local function strerror(e)
  ...
  if s == nil or (ffi.nullptr and s == ffi.nullptr) then return 'errno ' .. tostring(e) end
  return ffi.string(s)
end

local function wrap_error(ret)
  local n = tonumber_c(ret)
  if n == -1 then local e = errno(); return nil, strerror(e), e end
  return n, nil, nil
end
...
local function epoll_event_size() return ffi.sizeof('struct epoll_event') end
local function validate_epoll_event_abi()
  ...
  if size < 12 then return nil, 'unexpected epoll_event ABI size for ' .. tostring(ARCH) .. ': ' .. tostring(size) end
  return true
end
...
local function epoll_create()
  ...
  if ok_call then
    local fd = wrap_error(ret)
    if fd then return fd end
  end
  local fd, err = wrap_error(C.epoll_create(1))
  if not fd then error(err or 'epoll_create failed', 2) end
  return fd
end
...
function Linux.new(opts)
  opts = opts or {}
  local ok_abi, abi_err = validate_epoll_event_abi()
  if not ok_abi then error('fibers.host.cffi_linux: ' .. tostring(abi_err), 2) end
...
function Linux:sleep(seconds) return sleep_seconds(seconds) end
...
function Linux:_delete_withdrawn(by_fd)
  ...
  for fd in pairs(self.active) do if not by_fd[fd] then active_to_delete[#active_to_delete + 1] = fd end end
  for i = 1, #active_to_delete do local ok, err = self:_delete(active_to_delete[i]); if not ok then return nil, err end end
  local unpollable_to_delete = {}
  for fd in pairs(self.unpollable) do if not by_fd[fd] then unpollable_to_delete[#unpollable_to_delete + 1] = fd end end
...
function Linux:block(rt, waits, status, _opts)
  if not self.epfd then error('fibers.host.cffi_linux: host is closed', 2) end
  waits = waits or {}
