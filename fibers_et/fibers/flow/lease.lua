...
function Lease:bytes_value() return self:bytes() end
function Lease:length() return self._length or #(self._bytes or '') end
function Lease:length_value() return self:length() end
function Lease:inspect() return { id = self.id, bytes = self:bytes(), length = self:length(), owner = self.owner, reservoir = self.reservoir } end
