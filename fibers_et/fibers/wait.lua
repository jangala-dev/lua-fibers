...
function Wait.manual(source, interest)
  local sid = stable_source_id(source)
  local name = interest or 'ready'
  return make('manual', tostring(sid) .. ':' .. tostring(name), {
    source = source,
    interest = name,
...
function Wait.poll(key, mode, source)
  mode = mode or 'read'
  return make('poll', tostring(mode) .. ':' .. tostring(key), {
    poll_key = key,
    mode = mode,
    source = source,
    primitive = 'poll',
