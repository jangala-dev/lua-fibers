...
local Summary = require('fibers.kernel.algebra.summary')
...
local combine_seq_at_path = Candidate.combine_seq_at_path
local combine_parallel = Candidate.combine_parallel
local deferred_target_resolved = Candidate.deferred_target_resolved
local deferred_target_values = Candidate.deferred_target_values
local set_target_pack = Candidate.set_target_pack
local same_path = Candidate.same_path
local structural_clone = Candidate.structural_clone
...
local function first_ready_deferred(c)
  local ds = c.deferred or EMPTY_PREF
  for i = 1, #ds do
    if deferred_target_resolved(c, ds[i]) then return i, ds[i] end
  end
  return nil
end

local function split_deferred(ds, remove_i, path)
  local other, same_tail = {}, {}
  for i = 1, #ds do
    if i ~= remove_i then
      local d = ds[i]
      if i > remove_i and same_path(d.path, path) then
        same_tail[#same_tail + 1] = d
      else
        other[#other + 1] = d
      end
    end
  end
  return other, same_tail
end

local function process_one_deferred(c, ctx)
  if #c.deferred == 0 then return nil end
  local idx, d = first_ready_deferred(c)
  if not d then return nil end

  local vals = deferred_target_values(c, d)
  if not vals then return nil end
  local other, tail = split_deferred(c.deferred, idx, d.path)
  c.deferred = other
  local dctx = d.ctx or ctx
  if d.kind == 'map' then
    if c.post then error('map cannot consume a post-commit wrapped value', 2) end
    local new_vals = pack_(call_callback(dctx, 'map', 'callback_error', d.fn, unpack_(vals, 1, vals.n)))
    if not set_target_pack(c, d.path, new_vals) then return nil end
    for i = 1, #tail do c.deferred[#c.deferred + 1] = tail[i] end
  ...
  elseif d.kind == 'bind' then
    ...
    local out = {}
    for i = 1, #r.cands do
      out[#out + 1] = combine_seq_at_path(c, r.cands[i], d.path, tail)
    end
...
local function normalise_result(r, ctx)
  ...
  for i = 1, #cands do
    if first_ready_deferred(cands[i]) then
      runnable = true
      break
    end
  end
...
local function eval_product(node, ctx, allow_internal)
  ...
  local function rec(i, acc, rows)
    ...
    for j = 1, #lane_lists[i] do
      ...
      local merged = combine_parallel(acc, lane, i)
      if merged then
        local nr = list_copy(rows)
        nr._fibers_rows = true
        nr[i] = structural_clone(lane.vals)
  ...
  elseif k == 'bind' then
    ...
    for i = 1, #r.cands do
      ...
      if raw_resolved(c.vals, c.subst) then
        ...
        c.deferred[#c.deferred + 1] = { kind = 'bind', fn = node.fn, ctx = child_ctx(ctx, 'bind:q'), path = nil }
        out[#out + 1] = c
      end
    end
