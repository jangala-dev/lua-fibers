...
local function mark_location(out, loc, fields)
  ...
  if not access then access = {}; out.locations[loc] = access end
  for key, value in pairs(fields or {}) do
    if value then access[key] = true end
  end
end
...
local function metadata_merge(dst, src)
  ...
  for loc, access in pairs(src.locations or {}) do mark_location(dst, loc, access) end
...
local function primitive_metadata(op, out)
  ...
  if kind == 'exchange' then
    ...
    if not roles then roles = {}; out.exchanges[p.resource] = roles end
    roles[p.role] = true
    return
  end
  ...
  if not p.location then return end
  ...
  if p.interest ~= nil or p.absence_check ~= nil then out.external = true end
end

local describe

local function metadata_from_hint(hint, seen)
  if hint == false then return empty_metadata() end
  if type(hint) ~= 'table' then
    local out = empty_metadata(); out.dynamic = true; return out
  end
  if hint.kind and hint._id then return describe(hint, seen) end
  if hint._fibers_dependencies then
    local cached = dependency_hint_cache[hint]
    if cached then return cached end
    local out = empty_metadata()
    for i = 1, #(hint.parts or {}) do metadata_merge(out, metadata_from_hint(hint.parts[i], seen)) end
    dependency_hint_cache[hint] = out
    return out
  end
  if hint.footprint ~= nil then return metadata_from_hint(hint.footprint, seen) end
  if hint.continuation ~= nil then return metadata_from_hint(hint.continuation, seen) end

  local out = empty_metadata()
  out.dynamic = hint.dynamic == true
  out.external = hint.external == true
  for resource, roles in pairs(hint.exchanges or {}) do
    local target = {}; out.exchanges[resource] = target
    for role, present in pairs(roles) do if present then target[role] = true end end
  end
  for loc, access in pairs(hint.locations or {}) do
    if access == true then mark_location(out, loc, { read = true, write = true, supply = true })
    else mark_location(out, loc, access) end
  end
  for resource, access in pairs(hint.resources or {}) do
    if access == true then mark_resource(out, resource, { observe = true })
    else mark_resource(out, resource, access) end
  end
  return out
end

describe = function(op, seen)
  if not op then return empty_metadata() end
  local cached = metadata_cache[op]
  if cached then return cached end
  seen = seen or {}
  if seen[op] then
    local recursive = empty_metadata(); recursive.dynamic = true; return recursive
  end
  seen[op] = true
  ...
  elseif kind == 'choice' then
    for i = 1, #(op.choices or {}) do metadata_merge(out, describe(op.choices[i], seen)) end
  elseif kind == 'product' then
    for i = 1, #(op.lanes or {}) do metadata_merge(out, describe(op.lanes[i], seen)) end
...
function M.metadata_covers(declared, actual)
  ...
  if declared.dynamic then return true end
  if actual.dynamic then return false, 'dynamic continuation' end
  if actual.external and not declared.external then return false, 'external dependency' end
  for resource, roles in pairs(actual.exchanges or {}) do
    local allowed = declared.exchanges and declared.exchanges[resource]
    if not allowed then return false, 'exchange resource ' .. tostring(resource) end
    for role in pairs(roles) do
      if not allowed[role] then return false, 'exchange role ' .. tostring(role) end
    end
  end
  for location, access in pairs(actual.locations or {}) do
    local allowed = declared.locations and declared.locations[location]
    if not allowed then return false, 'location ' .. tostring(location) end
    for mode, present in pairs(access) do
      if present and not allowed[mode] then return false, 'location mode ' .. tostring(mode) end
    end
  end
  for resource, access in pairs(actual.resources or {}) do
    local allowed = declared.resources and declared.resources[resource]
    if not allowed then return false, 'resource ' .. tostring(resource) end
    for mode, present in pairs(access) do
      if present and not allowed[mode] then return false, 'resource mode ' .. tostring(mode) end
    end
  end
  return true
end

function M.metadata(op) return describe(op) end
function M.footprint(op) return describe(op) end
...
function M.supply_score(metadata, intents)
  ...
  if metadata and metadata.dynamic then return math.max(1, #(intents or {})), 'dynamic' end
  for i = 1, #(intents or {}) do
    local ok, reason = M.metadata_may_supply(metadata, intents[i])
    if ok then score = score + 1; first_reason = first_reason or reason end
  end
...
function M.metadata_counts(metadata)
  ...
  for _ in pairs((metadata and metadata.exchanges) or {}) do exchanges = exchanges + 1 end
  for _ in pairs((metadata and metadata.locations) or {}) do locations = locations + 1 end
  for _ in pairs((metadata and metadata.resources) or {}) do resources = resources + 1 end
