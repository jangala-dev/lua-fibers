...
--   * provenance-aware projection for partial operations;
--   * generic validation and commit.
--
-- Facility modules construct locations and operations.  They do not define
...
function M.clone_patch(p)
  ...
  if p.kind == 'replace' then return { kind = 'replace', value = p.value } end
  if p.kind == 'add' then return { kind = 'add', delta = p.delta } end
...
function M.new_view(root_id, scope_path, source)
  local cells = {}
  if source then
    for loc, rec in pairs(source.cells or {}) do
      cells[loc] = { value = rec.value, version = rec.version }
    end
  end
  return {
...
function M.cell(view, loc, trail)
  ...
  if not rec then
    if trail then trail:snapshot_view(view) end
...
local function clone_map_value(loc, value)
  local out = {}
  for k, v in pairs(value or {}) do out[k] = loc.clone_value and loc.clone_value(v) or v end
  return out
end

function M.apply_patch_value(loc, value, patch)
  if patch.kind == 'replace' then return patch.value end
  if patch.kind == 'add' then return value + patch.delta end
  if patch.kind == 'presence' then
    for i = 1, #(patch.ops or {}) do
      local op = patch.ops[i]
      if op.op == 'put' then value = op.value
      elseif op.op == 'remove' or op.op == 'take' then value = M.ABSENT
      else error('unknown presence operation: ' .. tostring(op.op), 2) end
    end
    return value
  end
  ...
  if patch.kind == 'finite_map' then
    ...
    for i = 1, #(patch.ops or {}) do
      local op = patch.ops[i]
      if op.op == 'put' then out[op.key] = loc.clone_value and loc.clone_value(op.value) or op.value
      elseif op.op == 'remove' or op.op == 'take' then out[op.key] = nil
      else error('unknown finite-map operation: ' .. tostring(op.op), 2) end
    end
    return out
  end
...
function M.stage(view, loc, patch, trail)
  if trail then trail:snapshot_view(view) end
  local rec = M.cell(view, loc)
  local old = view.delta[loc]

  if loc.merge == 'replace' then
    if patch.kind ~= 'replace' then error('replace location requires replace patch', 2) end
    rec.value = patch.value
    view.delta[loc] = { kind = 'replace', value = patch.value }
    return true
  end

  if loc.merge == 'add' then
    if patch.kind ~= 'add' then error('add location requires add patch', 2) end
    rec.value = rec.value + patch.delta
    if old then old.delta = old.delta + patch.delta else view.delta[loc] = { kind = 'add', delta = patch.delta } end
    return true
  end

  if loc.merge == 'machine' then
    if patch.kind ~= 'machine' then error('machine location requires machine patch', 2) end
    if old then
      for i = 1, #(patch.steps or {}) do old.steps[#old.steps + 1] = patch.steps[i] end
      table.sort(old.steps, function(a, b) return a.serial < b.serial end)
    else
      view.delta[loc] = M.clone_patch(patch)
    end
    rec.value = M.apply_patch_value(loc, rec.value, patch)
    return true
  end

  if loc.merge == 'presence' or loc.merge == 'finite_map' then
    local expected = loc.merge
    if patch.kind ~= expected then error(expected .. ' location requires ' .. expected .. ' patch', 2) end
    rec.value = M.apply_patch_value(loc, rec.value, patch)
    if old then
      for i = 1, #(patch.ops or {}) do old.ops[#old.ops + 1] = patch.ops[i] end
    else
      view.delta[loc] = M.clone_patch(patch)
    end
    return true
...
function M.merge_parallel(loc, left, right, composition)
  if not left then return M.clone_patch(right) end
  if not right then return M.clone_patch(left) end
  ...
  if loc.merge == 'machine' then
    local steps = {}
    for i = 1, #(left.steps or {}) do steps[#steps + 1] = left.steps[i] end
    for i = 1, #(right.steps or {}) do steps[#steps + 1] = right.steps[i] end
    table.sort(steps, function(a, b) return a.serial < b.serial end)
    return { kind = 'machine', steps = steps }
  end

  if loc.merge == 'presence' then
    local function simple(p)
      if p.kind ~= 'presence' or #(p.ops or {}) ~= 1 then return nil end
      return p.ops[1]
    end
    local a, b = simple(left), simple(right)
    if a and b then
      if a.op == 'put' and b.op == 'put' then
        if a.value ~= b.value then return nil, 'presence-put-conflict' end
  ...
  if loc.merge == 'finite_map' then
    local function by_key(p)
      ...
      for i = 1, #(p.ops or {}) do
        local op = p.ops[i]
        local xs = out[op.key]
        if not xs then xs = {}; out[op.key] = xs end
        xs[#xs + 1] = op
      end
      return out
    end
    ...
    for k in pairs(rb) do keys[k] = true end
    local ops = {}
    ...
    for k in pairs(keys) do
      local a, b = la[k], rb[k]
      if not a then append(b)
      elseif not b then append(a)
      elseif #a == 1 and #b == 1 then
        local x, y = a[1], b[1]
        if x.op == 'put' and y.op == 'put' then
          if loc.put_equal and x.value == y.value then
            append({x})
          elseif (composition == 'interacting' or composition == 'external')
              and x.policy == 'overwrite' and y.policy == 'overwrite' then
  ...
  if loc.merge == 'replace' then
    if left.value ~= right.value then return nil, 'replace-conflict' end
...
local function patch_orientation_component(loc, patch, direction)
  ...
  if patch.kind == 'add' then
    ...
    if (direction == 'up' and is_up) or (direction == 'down' and is_down) then return M.clone_patch(patch) end
    return nil
  end
...
function M.constraint_projection(loc, patch, orientation)
  if not orientation then return M.clone_patch(patch) end
...
local function path_relation(root_id, path, other_root, other_path)
  if root_id ~= other_root then return 'external' end
  local n = math.min(#path, #other_path)
  for i = 1, n do
    local a, b = path[i], other_path[i]
    if a.group_id ~= b.group_id then return 'unrelated' end
    if a.lane ~= b.lane then
      return a.mode == 'interacting' and 'interacting' or 'independent'
    end
  end
  if #other_path < #path then return 'ancestor' end
  if #other_path == #path then return 'same' end
  return 'descendant'
end
...
function M.project(state, task, loc, orientation, trail)
  ...
  for view_id, view in pairs(state.views) do
    if view_id ~= task.view_id and not view.merged then
      ...
      if patch then
        ...
        if relation == 'external' or relation == 'interacting' then include_patch = patch
        elseif relation == 'independent' then include_patch = M.constraint_projection(loc, patch, orientation) end
        if include_patch then
          combined = M.merge_parallel(loc, combined, include_patch, relation == 'interacting' and 'interacting' or relation == 'external' and 'external' or 'independent')
          if not combined then return nil, nil, 'projection-conflict' end
          used[#used + 1] = view_id
        end
      end
    end
  ...
  if combined then value = M.apply_patch_value(loc, value, combined) end
  return value, used
end
...
function M.project_machine(state, task, loc, succeeds, supply_policy, trail)
  ...
  for i = 1, #steps do
    ...
    if supply_restricted then
      ...
      if before or not after then value = st.value end
    else
      value = st.value
    end
  end
...
function M.merge_views(parent, children, mode, trail)
  ...
  for i = 1, #children do
    ...
    for loc, rec in pairs(child.cells) do
      if not parent.cells[loc] then parent.cells[loc] = { value = rec.value, version = rec.version } end
    end
    for loc, patch in pairs(child.delta) do
      local merged, err = M.merge_parallel(loc, writes[loc], patch, mode or 'independent')
      if not merged then return false, err end
      writes[loc] = merged
    end
  end

  for loc, patch in pairs(writes) do M.stage(parent, loc, patch, trail) end
  for i = 1, #children do
    if trail then trail:set(children[i], 'merged', true) else children[i].merged = true end
  end
  return true
end
...
function M.collect_candidate(root_views)
  ...
  for i = 1, #root_views do
    ...
    for loc, rec in pairs(view.cells) do
      ...
      if old ~= nil and old ~= rec.version then return nil, nil, 'observation-conflict' end
    ...
    for loc, patch in pairs(view.delta) do
      ...
      if not merged then return nil, nil, err end
      writes[loc] = merged
    end
  end
...
function M.validate(observations)
  for loc, version in pairs(observations or {}) do
    if (loc.version or 0) ~= version then return false, 'stale-location' end
  end
  return true
end
...
function M.commit(writes)
  for loc, patch in pairs(writes or {}) do
    ...
    if loc.apply then loc.apply(new_value, loc) end
  end
end
...
function M.result_pack(program, value)
  local kind = program.result_kind or 'constant'
  if kind == 'constant' then return pack_(program.result_value) end
  if kind == 'identity' then return pack_(value) end
  if kind == 'presence_bool' then return pack_(value ~= M.ABSENT) end
  if kind == 'presence_value' then
    if value == M.ABSENT or (program.nil_sentinel and value == program.nil_sentinel) then return pack_(nil) end
    return pack_(value)
  end
  if kind == 'index_entry' then
    if value == nil then return pack_(nil) end
    return pack_({ key = value.key, rank = value.rank, value = value.value, seq = value.seq })
  end
  if kind == 'map_value' then return pack_(value) end
  if kind == 'scalar_snapshot' then return pack_({ value = value, version = program.location.version }) end
  if kind == 'counter_state' then
    local owner = program.owner
    return pack_({ value = value, min = owner.min, max = owner.max, version = program.location.version })
  end
  error('unknown programme result kind: ' .. tostring(kind), 0)
end

function M.predicate_holds(program, value)
  if program.predicate == 'present' then return value ~= M.ABSENT end
  if program.predicate == 'absent' then return value == M.ABSENT end
  if program.predicate == 'ge' then return value >= program.threshold end
  if program.predicate == 'map_present' then return value[program.key] ~= nil end
  if program.predicate == 'map_absent' then return value[program.key] == nil end
  error('unknown claim predicate: ' .. tostring(program.predicate), 0)
end

local function select_extreme(program, value)
  local candidates = {}
  for key, entry in pairs(value or {}) do candidates[#candidates + 1] = { key = key, entry = entry } end
...
function M.evaluate_claim(program, value)
  ...
  if not query and program.kind == 'conditional_claim' then
    query = { kind = 'predicate', predicate = program.predicate, threshold = program.threshold, key = program.key }
  end
  if not query then error('claim programme is missing query', 0) end
  local witness = value
  if query.kind == 'predicate' then
    if not M.predicate_holds(query, value) then return nil end
  elseif query.kind == 'extreme' then
    witness = select_extreme(query, value); if not witness then return nil end
  elseif query.kind == 'compatible_insert' then
    for owner, mode in pairs(value or {}) do
      if owner ~= query.key and not (modes_compatible(query.compatibility, query.value, mode)
          and modes_compatible(query.compatibility, mode, query.value)) then return nil end
    end
  else error('unknown claim query: ' .. tostring(query.kind), 0) end
  local transition = program.transition
  if not transition and program.kind == 'conditional_claim' then transition = { kind = 'static', patch = program.claim_patch } end
  local patch
  if transition then
    if transition.kind == 'static' then patch = transition.patch
    elseif transition.kind == 'take_witness' then patch = { kind = 'finite_map', ops = { { op = 'take', key = witness.key } } }
    elseif transition.kind == 'put' then patch = { kind = 'finite_map', ops = { {
      op = 'put', key = transition.key, value = transition.value, policy = transition.policy,
    } } }
    else error('unknown claim transition: ' .. tostring(transition.kind), 0) end
  end
