...
local function probe_yieldable_pcall()
  local co = coroutine.create(function()
    return native_pcall(function()
      coroutine.yield('fibers.kernel.protected.probe')
      return 'ok'
    end)
  end)

  local ok, yielded = coroutine.resume(co)
  if not ok or yielded ~= 'fibers.kernel.protected.probe' or coroutine.status(co) ~= 'suspended' then
    return false
  end
