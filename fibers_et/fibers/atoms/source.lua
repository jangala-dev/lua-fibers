...
local function events_count(source) return source._validity:count() end
local function events_head_index(source) return source._validity.head or 1 end
...
function SourceKind.merge_par(dst, src)
  if src.head ~= nil and dst.head == nil then dst.head = src.head end
  if (dst.take or 0) > 0 and (src.take or 0) > 0 then return false, 'source-events-parallel-consume-conflict' end
  dst.take = (dst.take or 0) + (src.take or 0)
  return true
end

function SourceKind.project(source, rec, query)
  if source.kind ~= 'events' then return nil, false end
  local take = rec and rec.take or 0
  if query == 'count' then return events_count(source) - take, true end
...
function SourceKind.prepare(source, rec, _resolve)
  ...
  if source.kind ~= 'events' then return nil, 'source-consume-non-events' end
  if rec.head ~= nil and events_head_index(source) ~= rec.head then return nil, 'stale' end
  if events_count(source) < take then return nil, 'stale' end
  return { kind = SourceKind, resource = source, take = take, head = rec.head or events_head_index(source) }
end

function SourceKind.apply(prepared, _log)
  local ok, err = prepared.resource._validity:take(prepared.take or 0, 'events head consumed')
  if not ok then error(err or 'events consume failed') end
end
...
local function events_next(source, _payload, ctx)
  ...
  if value == nil then return Result.wait(Wait.source(source, 'next', { kind = 'events' })) end
  local c = Proposal.new(value)
  local r = ensure_source(c, source)
  r.head = r.head or events_head_index(source)
...
function SourceKind.eval(source, payload, ctx)
  ...
  elseif source.kind == 'events' then
    if op ~= 'next' then error('events sources support next_op, not wait_op', 2) end
    return events_next(source, payload, ctx)
...
function SourceKind.absence(source, payload, ctx)
  ...
  elseif source.kind == 'events' and op == 'next' then
    if source._validity:peek(ctx, 0) == nil then
      local frontier = source._validity:frontier_for('events.empty')
      if ctx and ctx.add then ctx:add({ kind = 'events-empty', source = source, frontier = frontier, stamp = frontier and frontier.gen or nil }) end
      return true
    end
...
function Source.events(name) return new_source('events', { name = name }) end
...
function Source:next_op()
  if self.kind == 'events' then return DefaultOp._resource(self, SourceKind, { op = 'next', interest = 'next' }) end
