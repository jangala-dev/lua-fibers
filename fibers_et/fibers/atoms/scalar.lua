...
function Scalar.kind(spec)
  ...
  function k:transition(name)
    ...
    if not t then error('unknown scalar transition ' .. tostring(name), 2) end
    return t
  end
  return k
end
...
local function new_scalar(value, name, merge)
  ...
  scalar._location = Substrate.new_location({
    name = scalar.name .. ':value', merge = merge or 'replace', domain = 'plain', value = value, owner = scalar,
    apply = function(v, loc) scalar.value = v; scalar.version = loc.version end,
  })
  return scalar
end

function Scalar.new(value, name) return new_scalar(value, name, 'replace') end
function Scalar.machine(value, name) return new_scalar(value, name, 'machine') end

function Scalar:read_op()
  return Op._resource(self, Kind, Program.read(self._location, 'identity'))
end

function Scalar:snapshot_op()
  return Op._resource(self, Kind, Program.read(self._location, 'scalar_snapshot'))
end

function Scalar:changed_op(version)
  return Op._resource(self, Kind, Program.version_wait(self._location, version))
end

function Scalar:expect_op(value)
  local scalar = self
  local t = Scalar.transition {
    name = self.name .. ':expect', mode = 'query', supply = 'none',
    step = function(current)
      if current ~= value then return Scalar.Wait end
...
function Scalar:unsafe_update_op(fn)
  ...
  local t = Scalar.transition { mode = 'update', step = function(current) return fn(current) end }
...
function Scalar:unsafe_select_op(fn)
  ...
  local t = Scalar.transition { mode = 'select', step = function(current) return fn(current) end }
  return self:transition_op(t, {})
end
...
function Scalar:write_op(value)
  if self._location.merge == 'machine' then
    local transition = Scalar.transition {
      mode = 'update', order = 0,
      step = function() return value, true end,
    }
    return self:transition_op(transition, {})
  end
  return Op._resource(self, Kind, Program.patch(self._location, { kind = 'replace', value = value }, 'constant', true))
end
...
function Scalar:transition_op(transition, payload)
  ...
  payload = payload or {}
  if self._location.merge == 'replace' then self._location.merge = 'machine' end
