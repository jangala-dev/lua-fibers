...
function Clock.new(name)
  ...
  local c = setmetatable({
    ...
    _fibers_id = 'clock-' .. tostring(next_id), _fibers_kind = Kind,
  }, Clock)
  c._location = Substrate.new_location({
    name = c.name .. ':observation', merge = 'machine', domain = 'external-clock',
    value = false, owner = c,
  })
  return c
end
...
function Clock:at_op(deadline)
  local transition = Scalar.transition({
    name = self.name .. ':at', mode = 'query', supply = 'none',
    step = function(_, _, ctx)
      local now = ctx.now()
      if now < deadline then return Scalar.Wait end
