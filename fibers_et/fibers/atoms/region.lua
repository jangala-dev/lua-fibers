...
local DefaultOp = require('fibers.atoms.op')
...
local function as_settlement_protocol(settle, label)
  return Settlement.protocol(settle, label)
end

local function is_owned_handle(item)
  return type(item) == 'table' and item._fibers_kind == Ownership.Kind and item._validity_opaque ~= nil
end

local function require_owned_handle(item, label)
  if not is_owned_handle(item) then
    error((label or 'owned value') .. ' expects an owned handle; use Region.handle(...) or a resource constructor', 3)
  end
  return item
end
...
local function owned_spec(item, settle, children, opts)
  ...
  require_owned_handle(item, 'Region.Owned.item')
  opts = opts or {}
  return {
...
local function sorted_roots(region, rec)
  local map = projected_owned(region, rec)
  local out = {}
  for item, record in pairs(map) do
    if record.parent == nil then out[#out + 1] = item end
  end
  table.sort(out, function(a, b)
    local ak = tostring((type(a) == 'table' and (a._fibers_id or a.name)) or a)
    local bk = tostring((type(b) == 'table' and (b._fibers_id or b.name)) or b)
...
local function projected_counts(region, rec)
  local map = projected_owned(region, rec)
  ...
  return owned, roots, claimed, failed
end
...
function RegionKind.project(region, rec, query)
  ...
  if query == 'snapshot' then
    ...
    local owned_count, root_count, claimed_count, settlement_failed_count = projected_counts(region, rec)
    return { sealed = sealed, open = not sealed, owned_count = owned_count, root_count = root_count, claimed_count = claimed_count, failed_count = settlement_failed_count, settlement_failed_count = settlement_failed_count, version = region.version or 0 }, true
  elseif query == 'members' then
    return sorted_owned(region, rec), true
  elseif query == 'roots' then
    return sorted_roots(region, rec), true
...
local function discharge_claim_candidate(region, claim, ctx)
...
local function fail_claim_candidate(region, claim, failure, ctx)
  if not Claim.is(claim) then return nil end
  if claim.region ~= region then return nil end
  local item = claim.root
  local current_owner = Resource.project(ctx, item, 'owner')
...
local function restore_claim_candidate(region, claim, ctx)
  if not Claim.is(claim) then return nil end
  if claim.region ~= region then return nil end
  local item = claim.root
  local current_owner = Resource.project(ctx, item, 'owner')
  ...
  for child, record in pairs(subtree) do
    ...
    updated.claim = nil
    updated.claim_id = nil
    updated.claim_purpose = nil
    updated.claim_reason = nil
    updated.settlement_failed = nil
    updated.settlement_error = nil
    updated.settlement_error_message = nil
    rrec.add[child] = updated
    rrec.remove[child] = nil
    read_owned(c, child)
  end
...
function Region:discharge_claim_op(claim) return DefaultOp._resource(self, RegionKind, { op = 'discharge_claim', claim = claim }) end
function Region:fail_claim_op(claim, err) return DefaultOp._resource(self, RegionKind, { op = 'fail_claim', claim = claim, error = err }) end
function Region:restore_claim_op(claim) return DefaultOp._resource(self, RegionKind, { op = 'restore_claim', claim = claim }) end
function Region:resolve_claim_op(claim, resolution) return DefaultOp._resource(self, RegionKind, { op = 'resolve_claim', claim = claim, resolution = resolution }) end
...
function Region:roots_op() return DefaultOp._resource(self, RegionKind, { op = 'roots' }) end
function Region:live_op(item) return DefaultOp._resource(self, RegionKind, { op = 'live', item = item }) end
