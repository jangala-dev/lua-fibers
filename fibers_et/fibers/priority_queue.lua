...
function PriorityQueue:put_op(priority, value)
  if priority == nil then error('priority queue put requires a priority', 2) end
...
function PriorityQueue:get_op()
  return self.items:pop_first_op():and_then(function(entry)
    ...
    return release:map(function() return entry.value, entry.rank end)
  end)
end
...
function PriorityQueue:snapshot_op()
  return self.items:snapshot_op():map(function(snapshot)
    local rows = {}
    for _, e in pairs(snapshot.entries or {}) do rows[#rows + 1] = { key = e.key, priority = e.rank, value = e.value, seq = e.seq } end
    table.sort(rows, function(a, b)
      if a.priority == b.priority then return (a.seq or 0) < (b.seq or 0) end
