...
function ScopeResult.is(x)
  return type(x) == 'table' and x._fibers_scope_result == true
end
function ScopeResult:unpack()
  if not self.ok then
    return nil, self.reason, self.report
  end
  return unpack_(self.values, 1, self.values.n or #self.values)
end
function ScopeResult:done_outcome()
...
function ScopeResult:tostring()
  if self.ok then
    return 'scope ok'
  end
  if self.report and ScopeReport.is(self.report) then
