...
local function same_cancellation(a, b)
  return Runtime.is_cancelled and Runtime.is_cancelled(a) and Runtime.is_cancelled(b)
    and a.token == b.token and a.reason == b.reason
end

local function perform_masked(scope, op)
  local rt = scope.runtime or Runtime.current()
  if not rt then error('masked scope operation requires a current runtime', 2) end
  return rt:perform(op, { masked = true })
end

local function monitor_perform(op)
  local rt = Runtime.current()
  if not rt then error('scope policy monitor requires a current runtime', 2) end
  return rt:perform(op, { masked = true })
end
...
local function record_child_exit(state, task, exit)
  if state.processed[task] then return nil end
...
local function add_active_task(scope, state, task)
  if not is_task(task) or state.active[task] then return end
  state.active[task] = true
  if state.processed[task] then return end
  ...
  if Exit.is(exit) then
    ...
    if entry then
      local decision = normalise_decision(call_policy(state.policy, 'on_child_exit', scope, state, task, exit), exit)
...
local function remove_active_task(state, task)
  if not state.active[task] then return end
...
local function record_active_exit(scope, state, task, exit)
  if not state.active[task] or state.processed[task] then return end
...
local function apply_lifetime_event(scope, state, event)
  ...
  if typ == 'admitted' then
    if event.to == scope.region then add_active_task(scope, state, task) end
  elseif typ == 'moved' then
    if event.from == scope.region then remove_active_task(state, task) end
    if event.to == scope.region then add_active_task(scope, state, task) end
...
local function monitor_loop(scope, state, policy)
  ...
  for task, record in pairs(scope.region.owned or {}) do
    if record.parent == nil then add_active_task(scope, state, task) end
  end
  ...
  while true do
    ...
    if body_is_done and state.pending_count == 0 then return state end
...
function Driver.start_monitor(scope, state, policy, rt)
  if state.monitor then return state.monitor end
...
local function default_result(scope, policy, state, body_ok, body_results, settlement_failures, close_reason)
  ...
  for i = 1, #state.child_failures do
    ...
    if err ~= primary then secondaries[#secondaries + 1] = err end
  end
  for i = 1, #settlement_failures do secondaries[#secondaries + 1] = settlement_failures[i] end
  ...
  if ScopeResult.is(custom) then return custom end
...
function Driver.run(scope, fn, policy)
  ...
  if not close_ok then settlement_failures[#settlement_failures + 1] = close_err end
  ...
  if not done_ok then settlement_failures[#settlement_failures + 1] = done_err end
  ...
  if not settle_ok then settlement_failures[#settlement_failures + 1] = settle_err end

  local result = default_result(scope, policy, state, body_ok, body_results, settlement_failures, close_reason)
