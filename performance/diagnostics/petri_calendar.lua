...
local function petri_cycle(n)
  ...
  fibers.run(function()
    for _ = 1, n do
      fibers.perform(net:fire_op(ab))
      fibers.perform(net:fire_op(ba))
    end
...
local function calendar_grow(n)
  ...
  fibers.run(function()
    for i = 1, n do
      fibers.perform(cal:reserve_at_op({ 'room' }, i, i + 1))
    end
  end, { quiet_deadlock = true })
end
