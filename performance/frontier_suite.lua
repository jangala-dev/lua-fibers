...
do
  ...
  for i = 1, 64 do
    ...
    rt:spawn_raw(function()
      rt:perform(cells[i]:expect_op(1))
    end)
  end
  assert(blocked(rt:run()))
  for i = 1, #rt.engine.pending do
    ids[i] = rt.engine.pending[i].id
  end
  rt:spawn_raw(function()
    rt:perform(cells[17]:write_op(1))
  end)
  rt:_start_one()
  local dirty = 0
  for i = 1, #ids do
    if rt.engine.proof_graph.dirty[ids[i]] then
      dirty = dirty + 1
    end
  end
...
for i = 1, #rows do
  local values = {}
  for j = 1, #headers do
    values[j] = tostring(rows[i][headers[j]] or 0)
  end
