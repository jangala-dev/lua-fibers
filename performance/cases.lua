...
add('simple', 'external', 'preloaded event queue', 1200, function(ctx, n)
  local rt = ctx:runtime()
  local queue = EventQueue.new():label('perf-events')
  ...
  end):label('perf-events-consumer')
  drain(rt)
  assert_eq(total, n * (n + 1) / 2)
  return n
end)
...
add('moderate', 'product', 'internal then external rendezvous', 260, function(ctx, n)
  local rt = ctx:runtime()
  local inside = Rendezvous.new():label('perf-product-inside')
  local outside = Rendezvous.new():label('perf-product-outside')
  local total = 0
  rt:spawn_raw(function()
    for i = 1, n do
  ...
  end):label('perf-product-main')
  rt:spawn_raw(function()
    for i = 1, n do
      rt:perform(outside:put_op(1000 + i))
    end
  end):label('perf-product-partner')
  drain(rt)
...
add('moderate', 'flow', 'sequential write read', 280, function(ctx, n)
  local rt = ctx:runtime()
  local flow = Flow.new(nil):label('perf-flow')
...
add('complex', 'search', 'triple swap with decoy', 14, function(ctx, n)
  ...
  for round = 1, n do
    local rt = ctx:runtime()
    local ab = Rendezvous.new():label('perf-ab-' .. tostring(round))
    local bc = Rendezvous.new():label('perf-bc-' .. tostring(round))
    local ca = Rendezvous.new():label('perf-ca-' .. tostring(round))
    local a, b, c
    rt:spawn_raw(function()
      a = rt:perform(Op.each({ ab:put_op('A'), ca:get_op() }):map(function(rows)
        return rows[2][1]
      end))
    end):label('perf-swap-a')
    rt:spawn_raw(function()
      b = rt:perform(Op.each({ bc:put_op('B'), ab:get_op() }):map(function(rows)
        return rows[2][1]
      end))
    end):label('perf-swap-b')
    rt:spawn_raw(function()
      c = rt:perform(Op.each({ ca:put_op('C'), bc:get_op() }):map(function(rows)
        return rows[2][1]
      end))
    end):label('perf-swap-c')
    rt:spawn_raw(function()
      rt:perform(ab:get_op())
    end):label('perf-decoy')
    drain(rt)
    assert_eq(a, 'C')
    assert_eq(b, 'A')
    assert_eq(c, 'B')
...
add('complex', 'search', 'contended producers', 6, function(ctx, rounds)
  ...
  for round = 1, rounds do
    local rt = ctx:runtime()
    local ch = Rendezvous.new():label('perf-contention-' .. tostring(round))
    local total = 0
    for producer = 1, producers do
      ...
      end):label('perf-producer-' .. tostring(producer))
    end
    rt:spawn_raw(function()
      for _ = 1, producers * messages do
        total = total + rt:perform(ch:get_op())
      end
    end):label('perf-contention-consumer')
    drain(rt)
    assert_ok(total > 0)
...
add('complex', 'search', 'nursery rendezvous fanout seven', 1, function(ctx, rounds)
  ...
  for round = 1, rounds do
    ...
    local result = fibers.try_run(
      function()
        local ch = Rendezvous.new():label('perf-nursery-fanout-' .. tostring(round))
        for i = 1, fanout do
          fibers.spawn(function()
            fibers.perform(ch:put_op(i))
          end):label('perf-child-' .. tostring(i))
        end
        for _ = 1, fanout do
