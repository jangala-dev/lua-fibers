...
local function expect_op(value, label)
  if not is_op(value) then error(label .. ' expects an Op', 3) end
  return value
end
...
function Op.choice(...)
  ...
  for i = 1, select('#', ...) do
    ...
    if choice.kind == 'choice' then
      for j = 1, #choice.choices do choices[#choices + 1] = choice.choices[j] end
    else
      choices[#choices + 1] = choice
    end
  end
  if #choices == 0 then return Op.never() end
  if #choices == 1 then return choices[1] end
  return node('choice', { choices = choices })
end

function Op.guard(fn)
  if type(fn) ~= 'function' then error('guard expects a function', 2) end
  return node('guard', { fn = fn })
end

function Op:map(fn)
  if type(fn) ~= 'function' then error('map expects a function', 2) end
...
local function product(mode, ...)
  local lanes = {}
  for i = 1, select('#', ...) do lanes[i] = expect_op(select(i, ...), mode) end
  if #lanes == 0 then return Op.always({ _rows = true }) end
...
function Op.add(location, delta)
  if type(delta) ~= 'number' then error('add expects a numeric delta', 2) end
  return node('add', { location = assert(location, 'add requires a location'), delta = delta })
end

function Op.take(location, amount)
  amount = amount or 1
  if type(amount) ~= 'number' or amount < 0 then error('take expects a non-negative amount', 2) end
...
function Op.at_least(location, threshold)
  if type(threshold) ~= 'number' then error('at_least expects a number', 2) end
  return node('transition', {
...
function Op.put(resource, value)
  return node('exchange', { resource = assert(resource, 'put requires a resource'), role = 'put', value = value })
end
...
local function join_patch(kind, left, right)
  if not left then return clone_patch(right) end
  if not right then return clone_patch(left) end
  if kind == 'add' then return { kind = 'add', delta = left.delta + right.delta } end
...
local function constraint_patch(patch, demand)
  if not patch then return nil end
  if patch.kind == 'replace' or demand == nil then return clone_patch(patch) end
  if demand == 'up' and patch.delta < 0 then return clone_patch(patch) end
  if demand == 'down' and patch.delta > 0 then return clone_patch(patch) end
  return nil
end

local function relation(left, right)
  if left == right then return 'same' end
  ...
  if a == b then
    return (left and left.depth or 0) > (right and right.depth or 0)
      and 'ancestor' or 'descendant'
  end
  while a and b and a.parent ~= b.parent do a, b = a.parent, b.parent end
  if not a or not b or a.group ~= b.group or a.lane == b.lane then return 'unrelated' end
...
local function clone_state(state)
  local out = {
    ...
    tasks = {}, segments = {}, groups = {},
    root_task = state.root_task,
    next_task = state.next_task, next_segment = state.next_segment, next_group = state.next_group,
    effects = copy_array(state.effects), trace = copy_array(state.trace),
  }
  for id, segment in pairs(state.segments) do
    local delta = {}
    for name, patch in pairs(segment.delta) do delta[name] = clone_patch(patch) end
    out.segments[id] = {
      parent = segment.parent, path = segment.path, delta = delta,
      retired = segment.retired,
    }
  end
  for id, group in pairs(state.groups) do
    local rows = {}
    for lane, values in pairs(group.rows) do rows[lane] = values end
    out.groups[id] = {
      parent_task = group.parent_task, parent_segment = group.parent_segment,
      mode = group.mode, count = group.count, completed = group.completed,
      child_segments = copy_array(group.child_segments), rows = rows,
    }
  end
  for id, task in pairs(state.tasks) do
    ...
    out.tasks[id] = {
      expr = task.expr, frames = frames, status = task.status,
      segment = task.segment, path = task.path,
      result = task.result, guard_input = task.guard_input,
    }
  end
  return out
end
...
local function projected_value(state, task, name, demand)
  ...
  for id, segment in pairs(state.segments) do
    ...
    if id ~= task.segment and not segment.retired and patch then
      ...
      local visible
      if rel == 'interacting' then visible = patch
      elseif rel == 'independent' then visible = constraint_patch(patch, demand) end
      if visible then
        local err
...
local function stage(state, segment_id, name, patch)
  ...
  if not summary then return false, err end
...
local function rows_pack(rows, count)
  ...
  for i = 1, count do value[i] = rows[i] end
  return pack(value)
end
...
local function join_group(state, group)
  ...
  for i = 1, group.count do
    ...
    for name, patch in pairs(segment.delta) do
      ...
      if not next_patch then return false, err end
  ...
  for i = 1, group.count do state.segments[group.child_segments[i]].retired = true end
  return true
end
...
local function start_product(state, task_id, expr)
  ...
  local group = {
    parent_task = task_id, parent_segment = task.segment,
    mode = expr.mode, count = #expr.lanes, completed = 0,
    child_segments = {}, rows = {},
  }
  ...
  for i = 1, #expr.lanes do
    ...
    state.tasks[child_id] = {
      expr = expr.lanes[i], frames = { { kind = 'lane', group = group_id, lane = i } },
      status = 'active', segment = segment_id, path = path,
...
local function resolve_exchange(state, left_id, right_id)
  ...
  if not complete_task(state, put_id, pack(true)) then return false end
...
local function encode(value, seen)
  if type(value) ~= 'table' then return scalar_key(value) end
  seen = seen or {}
  if seen[value] then return '<cycle>' end
  seen[value] = true
  local keys = {}
  for key in pairs(value) do keys[#keys + 1] = key end
  table.sort(keys, function(a, b) return scalar_key(a) < scalar_key(b) end)
  local out = { '{' }
  for i = 1, #keys do
    local key = keys[i]
...
local function search_branches(states, context)
  ...
  for i = 1, #states do
    ...
    for j = 1, #branch_hits do hits[#hits + 1] = branch_hits[j] end
...
search = function(state, context)
  ...
  if context.steps > context.max_steps then return {}, true end
  ...
  while true do
    ...
    if expr.kind == 'always' then
      if not complete_task(state, task_id, expr.values) then return {}, false end
    elseif expr.kind == 'guard' then
      local values = task.guard_input or pack()
      local residual = expr.fn(unpack_pack(values))
      if not is_op(residual) then error('guard callback must return a reference Op', 0) end
      task.expr = residual
    ...
    elseif expr.kind == 'or_else' then
      ...
      if #hits > 0 or unknown then return hits, unknown end
    ...
    elseif expr.kind == 'read' then
      if not complete_task(state, task_id, pack(own_value(state, task.segment, expr.location))) then return {}, false end
    elseif expr.kind == 'set' then
      if not stage(state, task.segment, expr.location, { kind = 'replace', value = expr.value }) then return {}, false end
      if not complete_task(state, task_id, pack(true)) then return {}, false end
    elseif expr.kind == 'add' then
      if not stage(state, task.segment, expr.location, { kind = 'add', delta = expr.delta }) then return {}, false end
      if not complete_task(state, task_id, pack(true)) then return {}, false end
    elseif expr.kind == 'emit' then
      state.effects[#state.effects + 1] = expr.effect
      if not complete_task(state, task_id, pack()) then return {}, false end
  ...
  if world then return { world }, false end
  ...
  for id = 1, state.next_task do
    ...
    if task and task.status == 'blocked' and task.expr.kind == 'transition' then
      ...
      if value ~= nil then
        ...
        if outcome then
          local branch = clone_state(state)
          if resolve_transition(branch, id, outcome) then branches[#branches + 1] = branch end
        end
      end
    end
  end
  ...
  for left_id = 1, state.next_task do
    ...
    if left and left.status == 'blocked' and left.expr.kind == 'exchange' then
      for right_id = left_id + 1, state.next_task do
        ...
        if right and right.status == 'blocked' and right.expr.kind == 'exchange'
          and exchange_compatible(left, right)
        then
          local branch = clone_state(state)
          if resolve_exchange(branch, left_id, right_id) then branches[#branches + 1] = branch end
        end
      end
    end
  end

  if #branches == 0 then return {}, false end
  return search_branches(branches, context)
end
...
function Reference.evaluate(op, options)
  ...
  local state = {
    ...
    tasks = {}, segments = {}, groups = {},
    root_task = 1, next_task = 1, next_segment = 1, next_group = 0,
    effects = {}, trace = {},
  }
  ...
  for i = 1, #worlds do
    ...
    if not unique[key] then unique[key] = true; deduped[#deduped + 1] = worlds[i] end
  end
  table.sort(deduped, function(a, b) return encode(a) < encode(b) end)
