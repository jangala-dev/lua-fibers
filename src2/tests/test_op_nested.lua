...
-- A "lease" ticket whose cap can be invalidated across yields.
  ...
  return op.new_primitive(
    function(self, _ctx, out)
      ...
      if not self.valid then
        return nil -- invalid cap (used for validation checks)
      end
...
do
  ...
  -- Fiendishly nested op:
  -- * Outer bracket wraps a guard which builds a choice.
  -- * Losing arm uses with_nack + bracket + never.
  -- * Winning arm uses bracket + and_then(lease -> rhs) with invalidation restart.
  -- * Winning arm then does all(always(y), choice(always('C'), never():finally(...))) and flattens.
  local nested =
