...
local function finish(parent, children)
  ...
  if not child then child = node(parent.context, parent); children[CHILD] = child end
  return child
end
...
function M.child(parent, ...)
  ...
  for i = 1, count do
    local key = select(i, ...)
    if key == nil then error('activation fact is required', 2) end
    local next_branch = children[key]
    if not next_branch then next_branch = {}; children[key] = next_branch end
...
function M.child_array(parent, head, values)
  ...
  if not next_branch then next_branch = {}; children[head] = next_branch end
  children = next_branch
  for i = 1, #(values or {}) do
    local key = values[i]
    local branch = children[key]
    if not branch then branch = {}; children[key] = branch end
    children = branch
  end
  return finish(parent, children)
end
...
local function provenance_relation(left, right)
  ...
  if a == b then return left and left.depth > (right and right.depth or 0) and 'ancestor' or 'descendant' end
...
local function normalise(input)
  if input == nil or (input.n or #input) == 0 then return EMPTY_INPUT end
  return input
end
...
function M.guard(runtime, request, guard, activation, reveal, input_pack)
  ...
  residual = runtime.runtime:_call_in_phase('guard', 'callback_error', guard.fn, unpack_(input, 1, input.n or #input))
