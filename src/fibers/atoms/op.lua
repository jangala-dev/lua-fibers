...
local function annotated(inner, post, defeat, symmetry_key)
  ...
  if symmetry_key ~= nil and existing_symmetry ~= nil and symmetry_key ~= existing_symmetry then
    error('an operation occurrence may carry only one certified symmetry key', 3)
  end
...
-- complete operation occurrence, including the fibre continuation after it
...
-- Compact trusted primitive used by high-frequency facilities.  The operation
-- node itself carries the immutable programme fields, avoiding a second table
-- between the operation and the evaluator.  General facilities may continue to
