...
local function current_errno()
  if type(nixio.errno) == 'function' then
    local ok, eno = pcall(nixio.errno)
    if ok then
      return tonumber(eno)
    end
  end
...
local function split_error(a, b)
  local eno, msg
  ...
  eno = eno or current_errno()
  return msg, eno
end

local function error_message(prefix, a, b)
  local msg, eno = split_error(a, b)
  if msg and msg ~= '' then
    return tostring(msg), eno
  end
  local text = eno and type(nixio.strerror) == 'function' and nixio.strerror(eno) or nil
  if text and text ~= '' then
    return tostring(text), eno
  end
  if eno then
    return tostring(prefix) .. ' (errno ' .. tostring(eno) .. ')', eno
  end
...
local function set_blocking(obj, blocking)
  if not obj or type(obj.setblocking) ~= 'function' then
    return true
  end
...
local function write_all(obj, bytes)
  ...
  while offset < #bytes do
    ...
    if type(n) == 'number' then
      if n <= 0 then
        return nil, 'short write', current_errno()
      end
      offset = offset + n
    elseif n == true then
      return true
...
local function preflight(spec, env)
  if type(spec.process_group) == 'number' then
    return nil,
      HostError.unsupported('process', 'process_group', {
        host = 'nixio',
  ...
  if spec.pass_fds and #spec.pass_fds > 0 then
    return nil,
      HostError.unsupported('process', 'pass_fds', {
        host = 'nixio',
  ...
  if program:find('/', 1, true) then
    ...
    if not candidate then
      return nil,
        HostError.system('process', 'exec', 'executable not found: ' .. program, 'ENOENT', const.ENOENT, {
          argv = spec.argv,
        })
    end
  end
...
local function reaper_main(spec, exec_path, env, stdio, inherited, status_r, status_w)
  ...
  if child_pid == 0 then
    ...
    if spec.cwd then
      ...
      if not ok then
        local _, eno = error_message('chdir failed', x, y)
        child_fail(launch_w, 'cwd', eno)
      end
    end
    if spec.new_session or spec.process_group == 'new' then
      local ok, x, y = nixio.setsid()
      if not ok then
        local _, eno = error_message('setsid failed', x, y)
        child_fail(launch_w, 'session', eno)
      end
    end
    local stdio_ok, _, stdio_eno = child_stdio_setup(stdio)
    if not stdio_ok then
      child_fail(launch_w, 'stdio', stdio_eno)
    end
...
local launch_actions = {
  ...
  fork = 'fork',
  pipe = 'pipe',
  handshake = 'exec_handshake',
  protocol = 'exec_handshake',
  reap = 'reap',
  status = 'reap',
}
...
local function parse_startup_line(line, spec)
  ...
  if stage then
    ...
    return nil,
      HostError.system('process', action, msg or (action .. ' failed'), nil, eno, { argv = spec.argv })
  end
  return nil,
    HostError.protocol('process', 'exec_handshake', 'invalid Nixio reaper launch response', {
      argv = spec.argv,
      payload = line,
    })
end
...
local function read_startup(status_r, spec)
  ...
  while true do
    ...
    if chunk == '' then
      return nil,
        nil,
        HostError.protocol('process', 'exec_handshake', 'Nixio reaper closed before reporting a child pid', {
          argv = spec.argv,
        })
    end
    buffer = buffer .. chunk
    if #buffer > 4096 then
      return nil,
        nil,
        HostError.protocol('process', 'exec_handshake', 'Nixio reaper launch response is too large', {
          argv = spec.argv,
        })
    end
...
function HostProcess:wait_op()
  ...
  if self.status_handle then
    return self.status_handle:read_ready_op()
  end
  return Sleep.sleep_op(self.poll_interval)
end
...
local function reap_reaper(self)
  ...
  if got == nil then
    ...
    return nil,
      HostError.system('process', 'reap', msg, nil, eno, {
        pid = self._pid,
...
function HostProcess:reap()
  ...
  while not self.status and not self.terminal_error do
    ...
    elseif HostError.is_eof(err) then
      self.terminal_error =
        HostError.protocol('process', 'reap', 'Nixio reaper closed without terminal status', {
          pid = self._pid,
        })
    else
...
function HostProcess:signal(value, target)
  ...
  if not ok then
    ...
    return nil,
      HostError.system('process', 'signal', msg, nil, eno, {
        pid = self._pid,
        signal = number,
        target = target,
      })
  ...
  if not blocking_ok then
    ...
    return nil,
      ...
      HostError.system(
        'process',
        'exec_handshake',
        blocking_msg or 'could not make launch sentinel blocking',
        nil,
        blocking_eno
      )
  end
