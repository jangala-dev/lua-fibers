...
local errno_names = {}
for _, name in ipairs({ 'EAGAIN', 'EWOULDBLOCK', 'EINTR', 'EINPROGRESS', 'EALREADY', 'EISCONN' }) do
  local value = errno[name]
  if type(value) == 'number' then
    errno_names[value] = name
  end
end

local function message(prefix, err, eno)
  if err and err ~= '' then
    return tostring(err)
  end
  if eno then
