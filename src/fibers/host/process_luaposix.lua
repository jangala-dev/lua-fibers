...
if not (ok_unistd and ok_fcntl and ok_errno and ok_signal and ok_wait and ok_stdlib)
    or type(unistd) ~= 'table' or type(fcntl) ~= 'table' or type(errno) ~= 'table'
    or type(signal) ~= 'table' or type(syswait) ~= 'table' or type(stdlib) ~= 'table' then
...
if not bit then return unsupported(bit_error) end
...
local function set_flag(fd, get_cmd, set_cmd, flag, enabled)
  ...
  if current == nil then return nil, err, eno end
  ...
  if ok == nil then return nil, set_err, set_eno end
  return true
end
...
local function close_fd(fd)
  if fd == nil or fd < 0 then return true end
  while true do
    local ok, err, eno = unistd.close(fd)
    if ok ~= nil then return true end
    if eno ~= errno.EINTR then return nil, err, eno end
  end
end

local function raw_pipe()
  local r, w, err, eno = unistd.pipe()
  if r == nil then return nil, nil, err, eno end
...
local function setup_environment_child(spec)
  if spec.env_mode == 'replace' then
    ...
    if current == nil then return nil, get_err, get_eno end
    for name in pairs(current) do
      local ok, err, eno = stdlib.setenv(name, nil)
      if ok == nil then return nil, err, eno end
    end
  end
...
local signal_numbers = {
  hup = signal.SIGHUP or 1, int = signal.SIGINT or 2, quit = signal.SIGQUIT or 3,
  kill = signal.SIGKILL or 9, usr1 = signal.SIGUSR1 or 10, usr2 = signal.SIGUSR2 or 12,
  pipe = signal.SIGPIPE or 13, alrm = signal.SIGALRM or 14, term = signal.SIGTERM or 15,
  chld = signal.SIGCHLD or 17, cont = signal.SIGCONT or 18, stop = signal.SIGSTOP or 19,
}
...
local signal_names = {
  [signal_numbers.hup] = 'HUP', [signal_numbers.int] = 'INT', [signal_numbers.quit] = 'QUIT',
  [signal_numbers.kill] = 'KILL', [signal_numbers.term] = 'TERM',
}
...
function HostProcess:wait_op()
  if self.status then return require('fibers.op').always(true) end
...
function HostProcess:reap()
  ...
  if pid == nil then
    ...
    return nil, HostError.system('process', 'reap', err_message('wait failed', err, eno), nil, eno, {
      pid = self._pid,
    })
  end
  ...
  elseif how == 'killed' or how == 'signaled' or how == 'signalled' then
    ...
    return nil, HostError.protocol('process', 'reap', 'unexpected wait status', {
      pid = self._pid, status = how, value = value,
    })
  end
  self.status = status
  self.reaped = true
...
function HostProcess:signal(signal_value, target)
  ...
  if ok == nil then
    return nil, HostError.system('process', 'signal', err_message('kill failed', err, eno), nil, eno, {
...
local function support_probe()
  local required = {
    unistd.fork, unistd.execp, unistd._exit, unistd.pipe, unistd.dup2, unistd.close,
    unistd.read, unistd.write, unistd.chdir, unistd.setpid, unistd.sysconf,
    fcntl.fcntl, fcntl.open, signal.kill, syswait.wait, stdlib.getenv, stdlib.setenv,
  }
  for i = 1, #required do if type(required[i]) ~= 'function' then return nil end end
  if fcntl.F_GETFD == nil or fcntl.F_SETFD == nil or not fcntl.FD_CLOEXEC
      or fcntl.F_GETFL == nil or fcntl.F_SETFL == nil or fcntl.O_NONBLOCK == nil
      or syswait.WNOHANG == nil or not Fd.is_supported() then return nil end
  return true
end
...
function Provider.support_reason() local ok, reason = Provider.is_supported(); return ok and nil or reason end
...
local function close_child_fds(spec, error_write)
  ...
  for _, value in ipairs(spec.pass_fds or {}) do
    ...
    if not fd or fd < 0 or fd ~= math.floor(fd) then return nil, nil, errno.EINVAL end
    if fd >= 3 and not keep[fd] then keep[fd] = true; ordered[#ordered + 1] = fd end
    if fd >= 3 then
      local ok, err, eno = set_cloexec(fd, false)
      if not ok then return nil, err, eno end
    end
  end
  if spec.close_fds == false then return true end
  table.sort(ordered)
  local max_fd = unistd.sysconf(unistd._SC_OPEN_MAX or 4)
  max_fd = tonumber(max_fd) or 1024
  for fd = 3, max_fd - 1 do if not keep[fd] then unistd.close(fd) end end
  return true
end
...
local function setup_stdio_child(stdio, error_write)
  ...
  local function install(which, target)
    ...
    elseif mode == 'null' then
      ...
      source, open_err, open_eno = fcntl.open(
        '/dev/null', which == 'stdin' and fcntl.O_RDONLY or fcntl.O_WRONLY, 0
      )
      if source == nil then return nil, open_err or 'open /dev/null failed', open_eno end
  ...
  for _, fd in ipairs(stdio.all_fds) do
    if fd ~= 0 and fd ~= 1 and fd ~= 2 and fd ~= error_write then unistd.close(fd) end
  end
  for i = 1, #opened do
    local fd = opened[i]
    if fd ~= 0 and fd ~= 1 and fd ~= 2 then unistd.close(fd) end
  end
  return true
end
...
local function build_argt(argv)
  local out = { [0] = argv[1] }
  for i = 2, #argv do out[i - 1] = argv[i] end
  return out
end
...
local launch_actions = {
  cwd = 'chdir', session = 'setsid', group = 'setpgid', environment = 'environment',
  stdio = 'stdio', close_fds = 'close_fds', exec = 'exec',
}
...
function Provider.start_process(host, spec)
  ...
  local stdio = { all_fds = {} }
  local parent_fds = {}
  local function add(fd) stdio.all_fds[#stdio.all_fds + 1] = fd end
  local function make_stdio(which, mode)
    stdio[which] = mode
    if mode ~= 'pipe' then return true end
    local r, w, err, eno = raw_pipe()
    if not r then return nil, HostError.system('process', 'pipe', err_message('pipe failed', err, eno), nil, eno, {
      stream = which,
    }) end
    add(r); add(w)
    if which == 'stdin' then stdio.stdin_child = r; parent_fds.stdin = w
    else parent_fds[which] = r; stdio[which .. '_child'] = w end
    return true
  end
  ...
  for _, which in ipairs({ 'stdin', 'stdout', 'stderr' }) do
    ...
    if not ok then
      for _, fd in ipairs(stdio.all_fds) do close_fd(fd) end
      return nil, nil, err
    end
  end
  ...
  if not error_read then
    for _, fd in ipairs(stdio.all_fds) do close_fd(fd) end
    return nil, nil, HostError.system('process', 'exec_pipe', err_message('pipe failed', pipe_err, pipe_eno), nil, pipe_eno)
  end
  ...
  if pid == nil then
    ...
    for _, fd in ipairs(stdio.all_fds) do close_fd(fd) end
    return nil, nil, HostError.system('process', 'fork', err_message('fork failed', fork_err, fork_eno), nil, fork_eno)
  end
  ...
  if pid == 0 then
    ...
    if spec.cwd then
      ...
      if ok == nil then child_fail(error_write, 'cwd', eno) end
    end
    if spec.new_session then
      local ok, _, eno = unistd.setpid('s', 0)
      if ok == nil then child_fail(error_write, 'session', eno) end
    elseif spec.process_group == 'new' then
      local ok, _, eno = unistd.setpid('p', 0, 0)
      if ok == nil then child_fail(error_write, 'group', eno) end
    ...
    if not env_ok then child_fail(error_write, 'environment', env_eno) end
    local stdio_ok, _, stdio_eno = setup_stdio_child(stdio, error_write)
    if not stdio_ok then child_fail(error_write, 'stdio', stdio_eno) end
    local close_ok, _, close_eno = close_child_fds(spec, error_write)
    if not close_ok then child_fail(error_write, 'close_fds', close_eno) end
  ...
  while true do
    ...
    if chunk ~= nil then
      ...
      if #handshake > 4096 then break end
    elseif read_eno ~= errno.EINTR then
      close_fd(error_read); kill_and_reap(pid)
      for _, fd in pairs(parent_fds) do close_fd(fd) end
      return nil, nil, HostError.system(
        'process', 'exec_handshake', err_message('handshake read failed', read_err, read_eno), nil, read_eno
      )
    end
  end
  close_fd(error_read)
  ...
  if handshake ~= '' then
    ...
    if not stage then
      return nil, nil, HostError.protocol('process', 'exec_handshake', 'invalid child setup failure', {
        argv = spec.argv, payload = handshake,
      })
    end
    local eno = tonumber(number)
    local action = launch_actions[stage] or 'exec_handshake'
    return nil, nil, HostError.system('process', action, err_message(action .. ' failed', nil, eno), nil, eno, {
      argv = spec.argv,
    })
  end
