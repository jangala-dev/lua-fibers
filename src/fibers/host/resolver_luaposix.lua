...
if not ok_socket or type(socket) ~= 'table' then return unsupported('requires posix.sys.socket') end
...
local function address_from(value, service)
  ...
  if family == socket.AF_INET then
    return {
      kind = 'inet4', family = 'inet4', host = value.addr or value.host,
      port = tonumber(value.port) or tonumber(service) or 0,
    }
  end
  if family == socket.AF_INET6 then
    return {
      kind = 'inet6', family = 'inet6', host = value.addr or value.host,
      port = tonumber(value.port) or tonumber(service) or 0,
...
function Resolver.resolve(_host, endpoint, opts)
  ...
  local family_value
  if family == 'inet4' then family_value = socket.AF_INET
  elseif family == 'inet6' then family_value = socket.AF_INET6
  else family_value = socket.AF_UNSPEC or 0 end
  ...
  if records == nil then
    return nil, HostError.system('resolver', 'resolve', tostring(err or 'address resolution failed'), nil, eno, {
      endpoint = endpoint,
    })
  end
