...
local function would_block(action, fields) return HostError.would_block('socket', action, fields) end
local function kind(address) return address and (address.kind or address.family) end

local function nixio_address(address)
  local family = kind(address)
  if family == 'inet4' then return 'inet', address.host, tonumber(address.port) end
  ...
  if family == 'unix' then return 'unix', address.path, 0 end
...
local function address_from(family, host, port)
  ...
  if family == 'inet6' then
    return { kind = 'inet6', family = 'inet6', host = host, port = tonumber(port) or 0, flowinfo = 0, scope_id = 0 }
  end
  if family == 'unix' then return { kind = 'unix', family = 'unix', path = host } end
  return nil
end
...
local function query_address(obj, peer, family)
  ...
  if not ok or host == nil then return nil end
  return address_from(family, host, port)
end

local function set_option(obj, level, option, value, action, fields)
  if type(obj.setopt) ~= 'function' then return nil, HostError.unsupported('socket', action, fields) end
  value = NixioError.option(value)
  local ok, a, b = obj:setopt(level, option, value)
  if ok == nil or ok == false then return nil, system_error(action, a, b, fields) end
  return true
end
...
local function wrap_socket(obj, host, name, family)
  ...
  handle.local_address = function(self) return query_address(self.obj, false, family) end
  handle.peer_address_value = function(self) return query_address(self.obj, true, family) end
  return handle
end
...
local function probe_family(family)
  if type(nixio.socket) ~= 'function' or not Fd.is_supported() then return false end
  local ok, obj = pcall(nixio.socket, family, 'stream')
  if not ok or not obj then return false end
  local supported = type(obj.bind) == 'function' and type(obj.listen) == 'function'
    and type(obj.accept) == 'function' and type(obj.connect) == 'function'
    and type(obj.getopt) == 'function' and type(obj.getsockname) == 'function'
...
local function supports(family)
  if support_cache[family] == nil then support_cache[family] = probe_family(family) end
  return support_cache[family]
end

function Socket.supports_ipv4() return supports('inet') end
function Socket.supports_ipv6() return supports('inet6') end
function Socket.supports_unix() return supports('unix') end
function Socket.is_supported() return Socket.supports_ipv4() or Socket.supports_ipv6() or Socket.supports_unix() end
function Socket.support_reason() return Socket.is_supported() and nil or 'required Nixio stream socket functions unavailable' end

function Socket.create_listener(host, address, opts)
  opts = opts or {}
  local family, bind_host, bind_port, address_err = nixio_address(address)
  if not family then return nil, address_err end
  if not supports(family) then return nil, HostError.unsupported('socket', 'listen', { address = address }) end
  local obj, a, b = nixio.socket(family, 'stream')
  if not obj then return nil, system_error('socket', a, b, { address = address }) end
  ...
  if family ~= 'unix' and opts.reuse_address ~= false then
    local ok, option_err = set_option(obj, 'socket', 'reuseaddr', true, 'setsockopt_reuseaddr', { address = address })
    if not ok then handle:close(option_err); return nil, option_err end
  end
  if family == 'unix' and opts.unlink_existing == true then os.remove(address.path) end
  ...
  handle._close = function(self, reason)
    ...
    if unix_path and opts.unlink_on_close ~= false then os.remove(unix_path) end
  ...
  handle.accept = function(self)
    ...
    while true do
      ...
      if child_obj then
        local child, child_err = wrap_socket(child_obj, host, (opts.name or 'listener') .. ':accepted', family)
...
function Socket.start_dial(host, address, opts)
  ...
  if not family then return nil, address_err end
  if not supports(family) then return nil, HostError.unsupported('socket', 'dial', { address = address }) end
  local obj, a, b = nixio.socket(family, 'stream')
  if not obj then return nil, system_error('socket', a, b, { address = address }) end
  ...
  if opts.local_address then
    ...
    if not local_family then handle:close(local_err); return nil, local_err end
  ...
  if family ~= 'unix' and opts.nodelay ~= false then
    local ok, option_err = set_option(obj, 'tcp', 'nodelay', true, 'setsockopt_nodelay', { address = address })
    if not ok then handle:close(option_err); return nil, option_err end
  end
  ...
  handle.finish_connect = function(self)
    if self._connect_complete then return self, query_address(self.obj, true, family) or address end
    self:clear_writable()
    local value, get_a, get_b = self.obj:getopt('socket', 'error')
    if value == nil then return nil, nil, system_error('connect_finish', get_a, get_b, { address = address }) end
