...
local function norm_error(a, b)
  ...
  return a or b, type(nixio.errno) == 'function' and nixio.errno() or nil
end

local function error_message(prefix, msg, eno)
  if msg and msg ~= '' then
    return tostring(msg)
  end
  local detail = eno and type(nixio.strerror) == 'function' and nixio.strerror(eno) or nil
  return detail and (tostring(prefix) .. ': ' .. tostring(detail))
    or (eno and (tostring(prefix) .. ' (errno ' .. tostring(eno) .. ')') or tostring(prefix))
end

local function system_error(action, a, b, fields)
  local msg, eno = norm_error(a, b)
  return HostError.system(
    'socket',
    action,
    error_message(action .. ' failed', msg, eno),
    errno_names[eno],
    eno,
    fields
  )
