-- One luaposix provider table.  This file contains only native bindings and
...
if not available then
  return {
    ...
    family = 'numeric-fd',
    available = false,
    reason = 'requires luaposix poll, time, errno, unistd, fcntl and socket modules',
  }
end
...
local function encode(address)
  ...
  if family == nil then
    return nil, require('fibers.host.error').invalid_argument('socket', 'address', { address = address })
  end
  if kind == 'unix' then
...
local provider = {
  ...
  capabilities = { datagram_truncation = false },
}
  ...
  return Direct.new({
    ...
    close_inherited = function(spec, error_write)
      ...
      for _, value in ipairs(spec.pass_fds or {}) do
        ...
        keep[fd] = true
        if fd >= 3 then
          local ok, eno = provider.fd.set_cloexec(fd, false)
          if not ok then
            return nil, eno
          end
        end
