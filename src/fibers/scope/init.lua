...
local function target_region(target)
  if is_scope(target) then
    return target.region
  end
  if is_region(target) then
    return target
  end
  return nil
end
...
local function policy_allows(scope, method, flag, ...)
  ...
  if flag and policy[flag] == false then
    return false, method .. ' denied by scope policy'
  end
  return true
end
...
local function require_policy(scope, method, flag, ...)
  ...
  if not ok then
    error(reason or (method .. ' denied by scope policy'), 3)
  end
end
...
function Scope.new(name, opts)
  ...
  if not is_region(region) then
    error('Scope.new expects opts.region to be a Region', 2)
  end
  ...
  local scope = setmetatable({
    ...
    cancellation = opts.cancellation
      or Scalar.new({ requested = false, cancelled = false }, (name or id) .. '-cancellation'),
  ...
  if region._fibers_scope_owner == nil then
    region._fibers_scope_owner = scope
  end
  return scope
end
...
function Scope:move_op(item, target)
  ...
  if not r then
    error('Scope:move_op expects a target Scope or Region', 2)
  end
  require_policy(self, 'allow_move', 'permit_outward_move', item, target)
  return self.region:move_op(item, r):map(function()
    return item
  end)
end
    ...
    local function scan(i)
      ...
      if Borrow.is(b) and b.subject == item and b:has_right(right) then
        return scope.region:record_op(b):and_then(function(record)
          if phase_live(record) then
            return Op.always(item, { kind = 'borrow', borrow = b, right = right })
          end
          return scan(i + 1)
        end)
      end
...
local function inherited_owner_scope(scope, item)
  ...
  if not owner_scope or owner_scope == scope then
    return nil
  end
  local p = scope.parent
  while p do
    if p == owner_scope then
      return owner_scope
    end
    p = p.parent
  end
...
function Scope:authorise_op(item, right)
  ...
  if owner_scope then
    return owner_scope:authorise_op(item, right)
  end
  return self.region
    :authorise_op(item, right, { allow_claimed = (self._settlement_depth or 0) > 0 })
...
local function parse_borrow_args(self, a, b, c)
  ...
  if not is_scope(borrower) then
    error('Scope:borrow_op expects a borrower Scope', 3)
  end
...
function Scope:begin_close_op(reason, opts)
  ...
  return self:task_roots_snapshot_op():and_then(function(snapshot)
    ...
    if opts.cancel_body ~= false then
      ops[#ops + 1] = self:request_cancel_op(reason)
    end
...
function Scope:sealed_op()
  return wait_state(self.sealed, function(v)
    return v == true
  end):map(function()
    return self
  end)
...
function Scope:_mark_done_op(result)
  ...
  return self.done:write_op({ status = 'done', outcome = outcome }):map(function()
    return outcome
  end)
end

function Scope:done_op()
  return wait_state(self.done, function(v)
    return type(v) == 'table' and v.status == 'done'
  end):map(function(v)
    return v.outcome
  end)
end

function Scope:owns_op(item)
  return self.region:owns_op(item)
end
function Scope:roots_op()
  return self.region:roots_op()
end
function Scope:record_op(item)
  return self.region:record_op(item)
end
function Scope:subtree_op(item)
  return self.region:subtree_op(item)
end
