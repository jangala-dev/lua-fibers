...
local function linear_compact(self)
  local hi = self.head_idx
  if hi <= 8 and hi <= (#self.chunks / 2) then
    return
  end
...
function LinearBuf_mt:read_avail()
  return self.len
end

function LinearBuf_mt:advance_read(n)
...
function LinearBuf_mt:find(pattern)
  ...
  while i <= last do
    local chunk = self.chunks[i]
    local part = (off > 0) and chunk:sub(off + 1) or chunk
    off = 0

    if #part > 0 then
      local combo = (carry ~= '') and (carry .. part) or part
      local pos = combo:find(pattern, 1, true)
      if pos then
        return (processed - #carry) + (pos - 1)
      end
