...
--
-- Linux epoll-based poller backend for the Pulse-only poller core.
-- Targeted wake-ups via EPOLLONESHOT; core re-arms after delivery by calling
-- ops.on_wait_change() for fired fds.
...
function Epoll:poll(timeout_ms)
  ...
  for fd, _ in pairs(evmap) do
    count = count + 1
    -- oneshot: treat as disarmed until re-added via on_wait_change
    self.active_events[fd] = nil
  end
...
-- Called by poller core:
--   * when interest transitions, and
--   * after delivery to re-arm oneshot.
...
-- Signature expected by Pulse poller core:
--   poll(backend, timeout_ms, rd_set, wr_set) -> events|nil
-- where rd_set/wr_set are plain sets: set[fd] == true when watched.
local function poll_backend(ep, timeout_ms, rd_set, wr_set)
  local events = {}

  -- Synthesise readiness for fds that epoll cannot watch (EPERM).
  -- If such fds exist and are watched, do not block in epoll_wait.
  local had_synthetic = false
  if ep.unpollable then
    for fd, _ in pairs(ep.unpollable) do
      local rd = (rd_set and rd_set[fd]) or false
      local wr = (wr_set and wr_set[fd]) or false
      if rd or wr then
        had_synthetic = true
