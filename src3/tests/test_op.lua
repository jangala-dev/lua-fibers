...
do
  ...
  runtime.main()
  assert(got == 'win')
  assert(loser_why == op.RB_ABORT, 'expected losing arm rollback reason RB_ABORT')
end
...
do
  ...
  local arm = op.with_nack(function(nack_op)
    runtime.spawn(function()
      local ok = op.perform(nack_op()) -- NackWait commits true when fired
      assert(ok == true)
      fired = true
    end, 'nack-waiter')
