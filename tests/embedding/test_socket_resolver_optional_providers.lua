...
do
  ...
  function socket_mod.listen()
    return 0
  end
  function socket_mod.getsockname(fd)
    return local_by_fd[fd]
  end
  function socket_mod.getpeername(fd)
    return peer_by_fd[fd]
  end
  ...
  function socket_mod.connect(fd, address)
    peer_by_fd[fd] = address
    return nil, 'in progress', 115
  end
  function socket_mod.getsockopt()
    return 0
  end
  ...
  with_modules({
    ...
    'fibers.host.socket_luaposix',
    'fibers.host.resolver_luaposix',
...
do
  ...
  local function object(family)
    ...
    local self = {
      ...
      local_port = 44000 + object_id,
    }
    function self:setblocking(value)
      assert_eq(value, false)
      return true
    end
    function self:setopt(_, _, value)
      assert_eq(type(value), 'number')
      return true
    end
    function self:bind(host, port)
      self.local_host, self.local_port = host, port == 0 and (44000 + self.id) or port
      return true
    end
    ...
    function self:getsockname()
      return self.local_host, self.local_port
    end
    function self:getpeername()
