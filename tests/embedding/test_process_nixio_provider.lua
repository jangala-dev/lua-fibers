...
local function fake_file(payload)
  ...
  return {
    ...
    closed = false,
    blocking = true,
    fileno = function() return fd end,
    setblocking = function(self, value) self.blocking = value; return true end,
    read = function(self, max)
      if self.closed then return nil, 9, 'closed' end
...
local fake_nixio = {
  ...
  stdin = fake_file(), stdout = fake_file(), stderr = fake_file(),
  errno = function() return 2 end,
  strerror = function(eno) return eno == 2 and 'not found' or ('errno ' .. tostring(eno)) end,
  fork = function() return 700 end,
  ...
  open = function() return fake_file() end,
  dup = function() return fake_file() end,
  kill = function(pid, signal) signals[#signals + 1] = { pid = pid, signal = signal }; return true end,
  chdir = function() return true end,
  getcwd = function() return '/work' end,
  getenv = function()
    return { PATH = '/bin:/usr/bin', HOME = '/home/test', REMOVE = 'yes' }
  end,
  setsid = function() return 1 end,
}
local fake_fs = {
  access = function(path)
    if path == '/bin/sh' or path == '/work/bin/tool' then return true end
    return nil, 2, 'not found'
  end,
  stat = function(path, field)
    if path == '/work' and field == 'type' then return 'dir' end
    return nil, 2, 'not found'
  end,
}
...
local ok, err = pcall(function()
  ...
  local found = false
  for _, obj in ipairs(Fd.open_objects()) do if obj == managed_obj then found = true end end
  assert(found)
  assert(managed:close())
  for _, obj in ipairs(Fd.open_objects()) do assert(obj ~= managed_obj) end

  local std_closed, other_closed = false, false
  Provider._test.close_inherited({
    { fileno = function() return 2 end, close = function() std_closed = true end },
    { fileno = function() return 9 end, close = function() other_closed = true end },
  })
  assert(std_closed == false and other_closed == true)

  local extended = assert(Provider._test.copy_environment({
    env_mode = 'extend', unset_env = { 'REMOVE' }, env = { EXTRA = 'present' },
  }))
  ...
  local replaced = assert(Provider._test.copy_environment({
    env_mode = 'replace', env = { ONLY = 'value' },
  }))
