...
local machine = 'kernel'
  ...
  do
    ...
    local result = run(Op.together({ guarded, guarded }))
    local rows = result[1]
    eq(calls, 2, machine .. ': together should activate both guard occurrences')
    eq(rows[1][1] == rows[2][1], false, machine .. ': separate together lanes should have independent activations')
  end
  ...
  do
    ...
    local result = run(Op.together({ sequence, sequence }))
    local rows = result[1]
  ...
  do
    ...
    local result = run(Op.choice(guarded, guarded))
  ...
  do
    local root = run(Op.guard(function(...)
      return Op.always(select('#', ...))
    end))
    eq(root.n, 1)
    eq(root[1], 0, machine .. ': root guard should receive no arguments')

    local zero = run(Op.always():and_then(Op.guard(function(...)
      return Op.always(select('#', ...))
    end)))
    eq(zero.n, 1)
    eq(zero[1], 0, machine .. ': zero-result prefix should supply no arguments')

    local sequenced = run(Op.always(nil, 'value'):and_then(Op.guard(function(...)
      local count = select('#', ...)
      local first, second = ...
  ...
  do
    ...
    local result = run(Op.always('primary'):or_else(fallback))
    eq(result[1], 'primary')
  ...
  do
    ...
    local rt = Runtime.new({ search_limit = 100 })
    local value
    rt:spawn_raw(function()
