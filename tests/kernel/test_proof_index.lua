...
do
  ...
  rt:spawn_raw(function()
    rt:perform(left:expect_op(1))
  end, 'left-waiter')
  rt:spawn_raw(function()
    rt:perform(right:expect_op(1))
  end, 'right-waiter')
...
do
  ...
  for _, request in ipairs(rt.engine.pending) do
    if request._retained_search then
      retained, retained_count = request._retained_search, retained_count + 1
    end
  end
  ...
  assert(
    coroutine.status(thread) == 'dead' or retained.disposed,
    'retained coroutine should finish rather than restart'
  )
