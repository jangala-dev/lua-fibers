...
local function dispatch(wrapper, label, count)
  ...
  for worker = 1, count do
    ...
    rt:spawn_raw(function()
      received[id] = rt:perform(channels[id]:get_op())
    end, label .. '-worker-' .. id)
  end
  rt:spawn_raw(function()
    local jobs = {}
...
dispatch(function(op)
  return op
end, 'plain-choice-propagation', 6)
dispatch(function(op)
  return op:map(function(value)
    return value
  end)
end, 'mapped-choice-propagation', 5)
dispatch(function(op)
  return op:wrap(function(value)
    return value
  end)
end, 'wrapped-choice-propagation', 5)
...
do
  ...
  local got_a, got_b
  rt:spawn_raw(function()
    got_a = rt:perform(a:get_op())
  end, 'get-a')
  rt:spawn_raw(function()
    got_b = rt:perform(b:get_op())
  end, 'get-b')
  rt:spawn_raw(function()
    rt:perform(Op.each({
      Op.choice(
