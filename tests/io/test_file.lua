...
function tests.evented_regular_file_round_trip()
  ...
  fibers.run(function()
    local opened = assert(file.open('/config', 'w+b', { provider = provider }))
    ...
    assert(file.read_all('/config', { max = 16, provider = provider }) == 'alpha123')
    assert(file.rename('/config', '/renamed', { provider = provider }))
    assert(file.unlink('/renamed', { provider = provider }))
    assert(file.mkdir('/directory', { provider = provider }))
    assert(file.mkdir_p('/directory/a/b', { provider = provider }))
  end, { host = Host.manual({ auto_advance_time = true }) })
end

function tests.file_ops_return_values_and_submissions_are_explicit()
  local provider = memory_provider({ ['/x'] = 'value' })
  fibers.run(function()
    assert(fibers.perform(file.read_all_op('/x', { provider = provider, max = 32 })) == 'value')
    local job = fibers.perform(file.submit_read_all_op('/x', { provider = provider, max = 32 }))
    assert(job and fibers.perform(job:result_op()) == 'value')

    local opened = assert(file.open('/x', 'rb', { provider = provider }))
...
function tests.read_line_eof_is_successful_nil()
  ...
  fibers.run(function()
    local opened = assert(file.open('/lines', 'rb', { provider = provider }))
...
function tests.bounded_handle_read_restores_position()
  ...
  fibers.run(function()
    local opened = assert(file.open('/bounded', 'rb', { provider = provider }))
...
function tests.append_and_read_limit_validation()
  ...
  fibers.run(function()
    assert(file.write_all('/append', 'b', { append = true, provider = provider }) == 1)
    assert(file.read_all('/append', { provider = provider }) == 'ab')
    local ok = pcall(file.read_all, '/append', { max = -1, provider = provider })
    assert(ok == false)
    ok = pcall(file.read_all, '/append', { chunk_size = 0, provider = provider })
    assert(ok == false)
  end, { host = Host.manual({ auto_advance_time = true }) })
end
...
function tests.write_all_retries_partial_writes()
  ...
  fibers.run(function()
    assert(file.write_all('/partial', 'abcdef', { provider = provider }) == 6)
    assert(file.read_all('/partial', { provider = provider, max = 16 }) == 'abcdef')
  end, { host = Host.manual({ auto_advance_time = true }) })
end
...
function tests.queued_operations_preserve_order_and_close_is_idempotent()
  local provider = memory_provider()
  fibers.run(function()
    local opened = assert(file.open('/ordered', 'w+b', { provider = provider }))
...
function tests.zero_length_and_line_boundaries()
  ...
  fibers.run(function()
    local opened = assert(file.open('/lines', 'r+b', { provider = provider }))
...
function tests.read_exactly_loops_and_reports_short_eof()
  ...
  provider.max_read = 2
  fibers.run(function()
    local opened = assert(file.open('/exact', 'rb', { provider = provider }))
...
function tests.flush_sync_and_permissions_are_distinct()
  local provider = memory_provider()
  fibers.run(function()
    local opened = assert(file.open('/durable', 'w+b', { provider = provider, permissions = 384 }))
    ...
    assert(opened:close())
    assert(file.mkdir('/private', { provider = provider, permissions = 448 }))
...
function tests.temporary_files_unlink_or_persist_after_rename()
  ...
  fibers.run(function()
    local temporary = assert(file.tmpfile({ provider = provider, directory = '/tmp', prefix = 'test-' }))
    ...
    local persisted = assert(file.tmpfile({ provider = provider, directory = '/tmp', prefix = 'test-' }))
    assert(persisted:write('keep') == 4)
    assert(persisted:rename('/saved'))
    assert(persisted:close())
    assert(file.read_all('/saved', { provider = provider, max = 16 }) == 'keep')
  end, { host = Host.manual({ auto_advance_time = true }) })
end
...
function tests.memory_provider_keeps_open_inode_after_rename_and_unlink()
  ...
  fibers.run(function()
    local first = assert(file.open('/old', 'r+b', { provider = provider }))
    local second = assert(file.open('/old', 'rb', { provider = provider }))
    assert(first:rename('/new'))
    assert(file.unlink('/new', { provider = provider }))
...
function tests.file_driver_does_not_stop_other_fibres()
  ...
  fibers.run(function()
    ...
    assert(file.read_all('/slow', { provider = provider }) == 'ready')
    task:await()
    assert(ticked)
  end, { host = Host.manual({ auto_advance_time = true }) })
end
...
function tests.worker_file_provider_is_evented()
  ...
  local result = fibers.try_run(function()
    ...
    local provider = Worker.new(fibers.current_runtime(), {
      worker_script = './tests/support/file_worker_delayed.lua',
    })
    local ticked = false
    ...
    assert(file.read_all(path, { provider = provider, max = 32 }) == 'worker')
    ticker:await()
    assert(ticked)
  end, { host = host })
  if host.close then
