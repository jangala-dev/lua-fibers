...
rt2:spawn_raw(function()
  ...
  report = { rt2:perform(task:outcome_op()):raise() }
...
rt4:spawn_raw(function()
  ...
  rt4:perform(handed:outcome_op()):raise()
...
rt_match:spawn_raw(function()
  ...
  rt_match:perform(task_a:outcome_op()):raise()
  rt_match:perform(task_b:outcome_op()):raise()
...
do
  ...
  rt_filter:spawn_raw(function()
    ...
    rt_filter:perform(task_a:outcome_op()):raise()
    rt_filter:perform(task_b:outcome_op()):raise()
    assert(Lifetimes.state(task_a).phase == 'retired')
    assert(Lifetimes.state(task_b).phase == 'retired')
