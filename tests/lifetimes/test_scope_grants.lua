...
do
  ...
  fibers.run(function()
    ...
    holder:retire(grant, 'revoked')
    assert_eq(grant:retired(), grant, 'Grant:retired should return the closed Grant')
...
do
  ...
  fibers.run(function(owner)
    ...
    owner:retire(h, 'subject closed')
    after_close = fibers.perform(maybe(holder:can_op(h, 'read')))
    holder:retire(grant, 'cleanup')
  end)
...
do
  ...
  fibers.run(function(owner)
    ...
    holder:retire(upstream, 'cleanup')
    owner:retire(h, 'done')
  end)
