...
do
  ...
  assert_eq(
    Lifetime.of(h):current_state().custodian,
    nil,
    'root scope exit should retire the handed-off handle'
  )
...
do
  local h = { name = 'post-suspend-owned' }
  Lifetime.inert(h)
  local owned_inside
  fibers.run(function()
  ...
  assert_eq(
    Lifetime.of(h):current_state().custodian,
    nil,
    'post-suspension owned root should be retired on scope exit'
  )
end
