...
function Contract.exercise(name, host)
  local report = fibers.try_run(function()
    ...
    assert(
      result.status.kind == 'exited' and result.status.code == 7 and result.status.success == false,
      name .. ' non-zero status'
    )
    assert(nonzero:close())

    local replaced = assert(process
      .command({
        '/bin/sh',
        '-c',
        'printf "%s:%s" "${FIBERS_ONLY-unset}" "${HOME-unset}"',
        stdout = 'pipe',
        stderr = 'pipe',
        env_mode = 'replace',
        env = { FIBERS_ONLY = 'present' },
      })
      :start())
